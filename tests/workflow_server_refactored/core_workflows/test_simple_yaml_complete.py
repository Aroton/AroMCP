...
class TestSimpleWorkflowComplete:
    ...
    def test_complete_simple_workflow_execution(self, simple_workflow_setup):
        ...
        print(
            f"   Initial state: counter={initial_state['state']['counter']}, doubled={initial_state['computed']['doubled']}"
        )
        ...
        if "steps" in first_step_batch:
            ...
            is_state_update = (
                first_client_step["type"] == "mcp_call"
                and first_client_step.get("definition", {}).get("tool") == "workflow_state_update"
            )
            if is_state_update:
            ...
            state_update_found = any(
                step["type"] == "mcp_call" and step.get("definition", {}).get("tool") == "workflow_state_update"
                for step in server_completed
            )
            if state_update_found:
                ...
                assert (
                    current_state["state"]["counter"] == 5
                ), f"Counter should be 5, got {current_state['state']['counter']}"
                assert (
                    current_state["computed"]["doubled"] == 10
                ), f"Doubled should be 10, got {current_state['computed']['doubled']}"
            ...
            if (
                first_step["type"] == "mcp_call"
                and first_step.get("definition", {}).get("tool") == "workflow_state_update"
            ):
        ...
        print("✅ test:simple.yaml executed successfully!")
        print(
            f"   Final state: name={final_state['inputs']['name']}, counter={final_state['state']['counter']}, doubled={final_state['computed']['doubled']}"
        )
    ...
    def test_simple_workflow_state_consistency(self, simple_workflow_setup):
        ...
        for i, (label, state) in enumerate(state_history):
            print(
                f"State {i} ({label}): counter={state.get('state', {}).get('counter', 'N/A')}, doubled={state.get('computed', {}).get('doubled', 'N/A')}"
            )
            ...
            if (
                "computed" in state
                and "doubled" in state["computed"]
                and "state" in state
                and "counter" in state["state"]
            ):
                expected_doubled = state["state"]["counter"] * 2
                assert (
                    state["computed"]["doubled"] == expected_doubled
                ), f"Computed field inconsistent at {label}: expected {expected_doubled}, got {state['computed']['doubled']}"
