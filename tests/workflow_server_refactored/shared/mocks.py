...
class MockWorkflowExecutor:
    ...
    def start(self, workflow_def: Any, inputs: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    def get_next_step(self, workflow_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            "step": {
                "id": "mock_step_1",
                "type": "user_message", 
                "definition": {"message": "Mock step execution"}
            }
        }
...
def create_mock_workflow_definition(name: str, steps: Optional[List[Dict[str, Any]]] = None) -> Mock:
...
def create_mock_step_response(step_type: str, step_id: str, definition: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Create a mock step response."""
    return {
        "step": {
            "id": step_id,
            "type": step_type,
            "definition": definition or {"message": f"Mock {step_type} step"}
        }
    }


def create_mock_batch_response(steps: List[Dict[str, Any]], server_completed: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
    """Create a mock batched step response."""
    return {
        "steps": steps,
        "server_completed_steps": server_completed or []
    }