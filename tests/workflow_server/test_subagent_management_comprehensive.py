...
class TestSubAgentTimeoutHandling:
    ...
    def test_timeout_inheritance_from_parent(self):
        ...
        coordinator.record_subagent_completion(
            parent_id, "sequential_agent_0", actual_duration=100  # Took 100s instead of 60s
        )

        # Remaining agents should get adjusted timeouts
        adjusted_timeout = coordinator.get_adjusted_timeout(parent_id, "sequential_agent_1")
    ...
    def test_cascading_timeout_cancellation(self):
        ...
        sub_agents = {
            "level1_agent": {"timeout": 50, "children": ["level2_agent_1", "level2_agent_2"]},
            "level2_agent_1": {"timeout": 40, "children": ["level3_agent_1"]},
            "level2_agent_2": {"timeout": 40, "children": []},
            "level3_agent_1": {"timeout": 30, "children": []},
        }

        for agent_id, config in sub_agents.items():
            coordinator.register_subagent(
                parent_id=parent_id, agent_id=agent_id, timeout=config["timeout"], children=config["children"]
            )
...
class TestSubAgentFailureIsolation:
    ...
    def test_failure_isolation_between_agents(self):
        ...
        for agent_id in agents:
            context = isolator.create_isolated_context(parent_id=parent_id, agent_id=agent_id, isolation_level="strict")
    ...
    def test_state_isolation_mechanisms(self):
        ...
        agent1_state = isolator.create_isolated_state(
            ...
            agent_id="agent_1",
            access_rules={"read": ["shared_data.items"], "write": ["agent_results.agent_1"]},
        )
    ...
    def test_communication_channel_isolation(self):
        ...
        comm_manager.send_message("agent_1", {"type": "task", "data": "process_item_1"})

        comm_manager.send_message("agent_2", {"type": "task", "data": "process_item_2"})
        ...
        comm_manager.broadcast_message(parent_id, {"type": "status_update", "message": "Phase 1 complete"})
...
class TestSubAgentResourceCleanup:
    ...
    def test_automatic_resource_cleanup_on_failure(self):
        ...
        resources = [
            cleaner.track_resource(agent_id, "file", {"path": "/tmp/agent_temp_file.txt", "created_at": time.time()}),
            cleaner.track_resource(agent_id, "connection", {"conn": Mock(close=Mock()), "type": "database"}),
            cleaner.track_resource(agent_id, "memory", {"buffer": Mock(release=Mock()), "size": 1024 * 1024}),  # 1MB
            cleaner.track_resource(agent_id, "file", {"path": "/tmp/agent_output.json", "created_at": time.time()}),
        ]
    ...
    def test_resource_lifecycle_management(self):
        ...
        agent_context = lifecycle_manager.create_agent(
            agent_id=agent_id,
            parent_id="parent_workflow",
            resource_requirements={"estimated_memory": 50 * 1024 * 1024, "estimated_duration": 120},
        )
    ...
    def test_configurable_cleanup_policies(self):
        ...
        cleaner.set_cleanup_policy("immediate", {"trigger": "on_completion", "delay": 0, "preserve_on_error": False})

        cleaner.set_cleanup_policy(
            "delayed", {"trigger": "on_completion", "delay": 300, "preserve_on_error": True}  # 5 minutes
        )

        cleaner.set_cleanup_policy("manual", {"trigger": "manual_only", "preserve_on_error": True})
...
class TestSubAgentMonitoring:
    ...
    def test_comprehensive_subagent_metrics(self):
        ...
        collector.configure(
            {
                ...
                "metrics": ["execution_time", "memory_usage", "cpu_usage", "message_count", "error_rate", "state_size"],
                "aggregation": True,
            }
        )
        ...
        for i in range(10):
            for j, agent_id in enumerate(agents):
                collector.record_metric(agent_id, "memory_usage", 20 + j * 10 + i * 2)  # Growing memory
                collector.record_metric(agent_id, "cpu_usage", 30 + (j * 15) % 40)  # Varying CPU
        ...
        # Test metric history
        history = collector.get_metric_history("agent_1", "memory_usage", last_seconds=5)
    ...
    def test_subagent_performance_profiling(self):
        ...
        monitor.enable_profiling(
            {"profile_cpu": True, "profile_memory": True, "profile_io": True, "sampling_interval": 0.1}
        )
    ...
    def test_subagent_monitoring_dashboard(self):
        ...
        monitor.configure_dashboard(
            {
                ...
                "alert_thresholds": {"memory_usage_percent": 80, "error_rate": 0.1, "execution_time": 300},
            }
        )
        ...
        for i in range(100):
            ...
            monitor.update_progress(
                "data_processor", {"processed": i * 10, "total": 1000, "current_memory": 50 + i % 20, "errors": 0}
            )

            # Validator progress (slower)
            monitor.update_progress(
                "validator",
                {"processed": i * 8, "total": 1000, "current_memory": 60 + i % 30, "errors": i // 20},  # Some errors
            )

            if i > 50:  # Aggregator starts later
                monitor.update_progress(
                    "aggregator", {"processed": (i - 50) * 5, "total": 500, "current_memory": 40, "errors": 0}
                )
    ...
    return WorkflowDefinition(
        ...
        steps=[
            WorkflowStep(
                ...
                definition={"items": "[1, 2, 3]", "agent": {"instructions": "Process item {{ item }}"}},
            )
        ],
    )
