...
from aromcp.workflow_server.workflow.context import ExecutionContext, LoopState, StackFrame
...
class TestScopedPathValidation:
    ...
    def test_read_only_scope_validation(self):
        ...
        read_only_paths = ["loop.current_item", "loop.index", "loop.nested.value"]
...
class TestScopedStateUpdates:
    ...
    def test_this_scope_updates_workflow_state(self):
        ...
        updates = [{"path": "this.new_variable", "value": "test_value"}, {"path": "this.nested.field", "value": 42}]
    ...
    def test_global_scope_updates_execution_context(self):
        ...
        updates = [{"path": "global.counter", "value": 5}, {"path": "global.config.debug", "value": True}]
...
class TestScopedVariableIntegration:
    ...
    def test_execution_context_scoped_variables_method(self):
        ...
        frame = StackFrame(frame_id="test_frame", frame_type="workflow", local_variables={"local_var": "local_value"})
        self.context.push_frame(frame)

        # Create loop with variables
        loop_state = LoopState(
            loop_id="test_loop", loop_type="foreach", variable_bindings={"loop.item": "item1", "loop.index": 0}
        )
