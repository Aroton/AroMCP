...
class TestComplexValidationScenarios:
    ...
    def test_multi_field_validation_expressions(self):
        ...
        result = engine.validate("complex_form", {"password": "SecurePass123!", "password_confirm": "SecurePass123!"})
        assert result["valid"] == True

        result = engine.validate("complex_form", {"password": "SecurePass123!", "password_confirm": "DifferentPass"})
        assert result["valid"] == False
        assert "Passwords do not match" in result["errors"][0]["message"]

        # Test age consent logic
        result = engine.validate("complex_form", {"age": 16, "parent_consent": True})
        assert result["valid"] == True

        result = engine.validate("complex_form", {"age": 16, "parent_consent": False})
        assert result["valid"] == False
        assert "parent consent" in result["errors"][0]["message"]

        # Test date range validation
        result = engine.validate("complex_form", {"start_date": "2024-01-01", "end_date": "2024-12-31"})
        assert result["valid"] == True

        result = engine.validate("complex_form", {"start_date": "2024-12-31", "end_date": "2024-01-01"})
        assert result["valid"] == False

        # Test budget allocation
        result = engine.validate(
            "complex_form", {"total_budget": 1000, "amounts": [300, 400, 200]}  # Sum = 900, under budget
        )
        assert result["valid"] == True

        result = engine.validate(
            "complex_form", {"total_budget": 1000, "amounts": [400, 500, 300]}  # Sum = 1200, over budget
        )
        assert result["valid"] == False
    ...
    def test_dynamic_validation_expressions(self):
        ...
        context = {
            "user": {"role": "admin", "permissions": ["read", "write", "delete"], "department": "engineering"},
            "system": {"maintenance_mode": False, "business_hours": True, "current_date": "2024-01-15"},
            "workflow": {"stage": "approval", "previous_approvals": 2, "required_approvals": 3},
        }
        ...
        result = validator.validate_expression(validation, {"action": "delete", "approval_code": "APPROVED-123"})
        assert result["valid"] == True
        ...
        result = validator.validate_expression(
            array_validation,
            {"selected_items": [{"id": 1, "required_permission": "read"}, {"id": 2, "required_permission": "write"}]},
        )
        assert result["valid"] == True

        result = validator.validate_expression(
            array_validation,
            {"selected_items": [{"id": 3, "required_permission": "execute"}]},  # User doesn't have this
        )
        assert result["valid"] == False
    ...
    def test_stateful_validation_scenarios(self):
        ...
        workflow_state = {
            "user_attempts": {"login": 2, "verification": 1},
            "completed_steps": ["registration", "email_verification"],
            "user_data": {"account_type": "premium", "balance": 1000},
            "history": [
        ...
        result = validator.validate_with_state(
            "workflow_123", attempt_validation, {"username": "user", "password": "pass"}
        )
        ...
        balance_validation = {"expression": "input.amount <= state.user_data.balance", "error": "Insufficient balance"}

        result = validator.validate_with_state("workflow_123", balance_validation, {"amount": 500})
        assert result["valid"] == True

        result = validator.validate_with_state("workflow_123", balance_validation, {"amount": 1500})
        assert result["valid"] == False
        ...
        result = validator.validate_with_state(
            "workflow_123", progression_validation, {"action": "proceed_to_dashboard"}
        )
        assert result["valid"] == True
...
class TestAdvancedTimeoutHandling:
    ...
    def test_hierarchical_timeout_coordination(self):
        ...
        # Start interaction
        interaction_handle = manager.start_interaction("workflow_1", "approval_flow", "manager_approval")
    ...
    def test_context_specific_timeout_behaviors(self):
        ...
        manager.configure_timeout_behavior(
            "critical_approval",
            {"type": "strict", "grace_period": 0, "auto_action": "reject", "notify_on_timeout": True},
        )

        manager.configure_timeout_behavior(
            "optional_feedback", {"type": "flexible", "grace_period": 30, "auto_action": "skip", "save_partial": True}
        )

        manager.configure_timeout_behavior(
            "background_task", {"type": "soft", "auto_extend": True, "max_extensions": 3, "extension_duration": 60}
        )

        # Test strict timeout
        strict_interaction = manager.create_interaction(
            interaction_id="approval_1", timeout=30, behavior="critical_approval"
        )
        ...
        extending_interaction = manager.create_interaction(
            interaction_id="background_1", timeout=120, behavior="background_task"
        )
    ...
    def test_timeout_recovery_strategies(self):
        ...
        manager.configure_recovery_strategy(
            "retry_with_extension", {"retry_count": 2, "timeout_multiplier": 1.5, "preserve_context": True}
        )
        ...
        manager.configure_recovery_strategy(
            "fallback_to_default",
            {
                ...
                "default_values": {"approval": "pending_review", "reason": "timeout_occurred"},
            },
        )

        manager.configure_recovery_strategy(
            "escalate_to_admin", {"escalation_timeout": 300, "admin_notification": True, "preserve_history": True}
        )

        # Test retry with extension
        interaction = {"id": "approval_1", "timeout": 60, "attempts": 0, "recovery": "retry_with_extension"}
        ...
        interaction = {"id": "optional_input", "recovery": "fallback_to_default"}
...
class TestInteractionCoordination:
    ...
    def test_multi_step_interaction_flow(self):
            ...
            "steps": [
                {
                    ...
                    "timeout": 300,
                    "validation": "required",
                    "next": {"approved": "manager_review", "rejected": "end", "needs_info": "clarification"},
                },
                {"id": "clarification", "type": "user_message", "timeout": 600, "next": "initial_review"},
                {
                    "id": "manager_review",
                    "type": "user_input",
                    "timeout": 1800,
                ...
                {"id": "risk_assessment", "type": "automated", "timeout": 60},
                {"id": "compliance_check", "type": "automated", "timeout": 120},
                {
                    "id": "final_decision",
                    "type": "user_input",
        ...
        result = coordinator.complete_step(
            flow_instance["id"],
            "initial_review",
            {"decision": "approved", "comments": "Looks good, forwarding to manager"},
        )
        ...
        coordinator.complete_step(flow_instance["id"], "risk_assessment", {"risk_level": "medium", "flags": []})

        coordinator.complete_step(
            flow_instance["id"], "compliance_check", {"compliant": True, "notes": "All checks passed"}
        )

        coordinator.complete_step(
            flow_instance["id"], "manager_review", {"approved": True, "conditions": ["monthly_review"]}
        )
...
def create_test_workflow() -> WorkflowDefinition:
    ...
    return WorkflowDefinition(
        ...
        steps=[
            WorkflowStep(
                id="step1", type="user_input", definition={"prompt": "Test prompt", "validation": {"required": True}}
            )
        ],
    )
