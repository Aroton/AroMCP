...
class TestCompleteSubAgentFlow:
    ...
    def test_complete_sub_agent_workflow_execution(self):
        ...
        assert (
            parallel_step is not None
        ), f"Expected parallel_foreach step, but got: {[s['type'] for s in step1.get('steps', [target_step])]}"
    ...
    def test_debug_mode_parallel_foreach_expansion(self):
        ...
        try:
            ...
            for attempt in range(max_attempts):
                ...
                for step in steps:
                    ...
                    if "process_file.item" in step_id:
                        ...
                        print("     ✓ Found expanded sub-agent step!")
            ...
            assert (
                len(all_expanded_steps) > 0
            ), f"Debug mode should have expanded sub-agent steps after {max_attempts} attempts"
            ...
            print(f"   ✅ Found {len(all_expanded_steps)} expanded sub-agent steps")
            print("   ✅ Steps are returned one at a time (single-step debug mode)")
    ...
    def test_debug_mode_with_parallel_waiting_loops(self):
        ...
        try:
            ...
            sub_agent_steps = [
                WorkflowStep(id="process_item", type="user_message", definition={"message": "Processing {{ item }}"})
            ]
            ...
            workflow_steps = [
                WorkflowStep(
                    ...
                    definition={"items": "{{ state.items }}", "sub_agent_task": "process_task", "max_parallel": 2},
                ),
                ...
                WorkflowStep(
                    ...
                    definition={
                        ...
                        "max_iterations": 100,
                        "body": [{"id": "wait_step", "type": "shell_command", "command": "sleep 1"}],
                    },
                ),
                WorkflowStep(
                    id="completion_message", type="user_message", definition={"message": "All processing complete"}
                ),
            ]
            ...
            workflow_def = WorkflowDefinition(
                ...
                default_state={
                    "state": {"items": ["item1", "item2"], "processing_results": {}},
                    "computed": {},
                    "inputs": {},
                },
            )
            ...
            if step1 is None:
                ...
                assert (
                    "wait_for_completion" not in step_ids
                ), f"Waiting loop should be skipped in debug mode, but found: {step_ids}"
    ...
    def test_debug_mode_with_template_variables_and_defaults(self):
        ...
        try:
            ...
            max_attempts_input = InputDefinition(
                type="number", description="Maximum attempts", default=5, required=False
            )

            file_path_input = InputDefinition(type="string", description="File path to process", required=True)
            ...
            if step1 and "error" in step1:
                ...
                if while_loop_steps:
                    ...
                    assert (
                        "{{ max_attempts }}" not in condition
                    ), f"Template variable not replaced in condition: {condition}"
                    assert "{{ max_attempts }}" not in str(
                        max_iterations
                    ), f"Template variable not replaced in max_iterations: {max_iterations}"
                    assert (
                        "5" in str(max_iterations) or max_iterations == 5
                    ), f"Default value (5) not applied: {max_iterations}"
