...
class TestSubAgentManagerPrompts:
    ...
    def test_standard_prompt_generation_without_custom_template(self):
        ...
        sub_agent_task.inputs = {"file_path": input_file_path, "max_attempts": input_max_attempts}
        ...
        if not prompt_template:
            ...
            context = {"task_id": "{{ task_id }}", "item": "{{ item }}", "index": "{{ index }}", "total": "{{ total }}"}
    ...
    def test_json_escaping_in_inputs_formatting(self):
        ...
        sub_agent_task = Mock()
        sub_agent_task.inputs = {"special_param": input_with_special_chars}
