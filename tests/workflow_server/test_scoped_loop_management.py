...
class TestScopedLoopManagement:
    ...
    def setup_method(self):
        ...
        initial_state = WorkflowState(
            inputs=self.workflow_instance.inputs,
            state={},
            computed={}
        )
    ...
    def test_nested_loops_variable_isolation(self):
        ...
        outer_loop = LoopState(
            ...
            loop_id="outer_loop",
            items=outer_items,
            max_iterations=len(outer_items)
        )
    ...
    def test_expression_evaluation_with_loop_variables(self):
        ...
        scoped_context = self.step_processor._build_scoped_context(
            self.workflow_instance, state, context
        )
        
        # Test loop variable access in expressions
        result = self.expression_evaluator.evaluate(
            "loop.item.name", state, scoped_context
        )
        assert result == "file1.txt"
        
        result = self.expression_evaluator.evaluate(
            "loop.item.size", state, scoped_context
        )
        assert result == 100
        
        result = self.expression_evaluator.evaluate(
            "loop.index", state, scoped_context
        )
        assert result == 0
    ...
    def test_foreach_step_processor_integration(self):
        ...
        result = self.step_processor.process_foreach(
            self.workflow_instance, foreach_step, foreach_step.definition, 
            queue, state, context
        )
    ...
    def test_while_step_processor_integration(self):
        ...
        self.state_manager.update("test_workflow", [
                ...
                "body": [
                    {
                        "id": "body_step",
                        "type": "shell_command",
                        "command": "echo Iteration {{loop.iteration}}"
                    }
                ]
            }
        )
        ...
        result = self.step_processor.process_while_loop(
            self.workflow_instance, while_step, while_step.definition,
            queue, state, context
        )
    ...
    def test_nested_foreach_loops(self):
        ...
        inner_loop = LoopState(
            ...
            loop_id="inner",
            items=inner_items,
            max_iterations=len(inner_items)
        )
        inner_loop.prepare_for_iteration()
        context.enter_loop(inner_loop)
    ...
    def test_mixed_loop_types_nesting(self):
        ...
        # Inner foreach loop
        items = ["a", "b", "c"]
        foreach_loop = LoopState(
            loop_type="foreach",
            loop_id="foreach_inner",
            items=items,
            max_iterations=len(items)
        )
