...
from aromcp.workflow_server.debugging.serial_mode import SerialDebugMode
                        ...
                        "else": [
                            {
                                ...
                                "type": "agent_prompt",
                                "config": {
                                    "prompt": "No items processed successfully"
                                }
                            }
                        ]
                    }
        ...
        workflow_def = WorkflowDefinition(
            ...
            steps=[
                ...
                StepDefinition(
                    ...
                    type="agent_prompt",
                    config={
                        "prompt": "Final count: {{ state.counter }}"
                    }
                )
            ]
        )
...
class TestMonitoringIntegration:
    ...
    async def test_comprehensive_metrics_collection(self, monitored_system):
        ...
        workflow_def = WorkflowDefinition(
            ...
            steps=[
                ...
                StepDefinition(
                    ...
                    config={
                        "updates": {
                            "large_data": "[" + ",".join([f'"{i}"' for i in range(1000)]) + "]"
                        }
                    }
                ),
                StepDefinition(
                    ...
                    type="agent_prompt",
                    config={
                        "prompt": "Analyze {{ state.large_data }}"
                    }
                )
            ]
        )
        ...
        for step_id in ['cpu_intensive', 'memory_intensive', 'io_intensive']:
            step_metrics = next(
                (s for s in metrics['step_timings'] if s['step_id'] == step_id),
                None
            )
    ...
    async def test_monitoring_data_export(self, monitored_system):
        ...
        export_data = observability.export_metrics(
