...
class TestConcurrentWorkflowExecution:
    ...
    async def test_parallel_step_performance(self, performance_system):
        ...
        for test_case in test_cases:
            result = await executor.start_workflow(
                parallel_workflow,
                {"parallel_count": test_case["parallel_count"]}
            )
        ...
        return WorkflowDefinition(
            ...
            steps=[
                WorkflowStep(
                    ...
                    config={
                        "updates": {
                            "result": "{{ [i**2 for i in range(inputs.complexity)] | sum }}"
                        }
                    }
                ),
                WorkflowStep(
        ...
        for iteration in range(test_iterations):
            ...
            for i in range(workflows_per_iteration):
                await executor.start_workflow(
                    workflow_def,
                    {"size": 1000}  # 1KB of data per workflow
                )
