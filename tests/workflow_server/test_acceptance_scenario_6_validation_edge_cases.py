...
class TestAcceptanceScenario6ValidationEdgeCases:
    ...
    def test_missing_required_fields(self):
        ...
        test_cases = [
            # Missing name
            {
                "workflow": {"description": "Test workflow", "version": "1.0.0", "steps": []},
                "expected_errors": ["name"],
            },
            # Missing description
            {
                "workflow": {"name": "test:workflow", "version": "1.0.0", "steps": []},
                "expected_errors": ["description"],
            },
            # Missing version
            {
                "workflow": {"name": "test:workflow", "description": "Test workflow", "steps": []},
                "expected_errors": ["version"],
            },
            # Missing steps
            {
                "workflow": {"name": "test:workflow", "description": "Test workflow", "version": "1.0.0"},
                "expected_errors": ["steps"],
            },
            # Multiple missing fields
            {"workflow": {"description": "Test workflow"}, "expected_errors": ["name", "version", "steps"]},
            # Empty workflow
            {"workflow": {}, "expected_errors": ["name", "description", "version", "steps"]},
        ]
        ...
        for i, test_case in enumerate(test_cases):
            ...
            for expected_field in test_case["expected_errors"]:
                assert (
                    expected_field in error_text
                ), f"Test case {i}: Missing field '{expected_field}' not reported in errors: {validator.errors}"
    ...
    def test_invalid_variable_references(self):
        ...
        test_cases = [
            ...
            # Typo in computed field - should be caught since computed fields are exact
            {
                "workflow": {
                    ...
                    "state_schema": {"computed": {"doubled_value": {"from": "state.value", "transform": "input * 2"}}},
                    "steps": [
                        {
                            "id": "step1",
                            "type": "user_message",
            ...
            # Missing input parameter - should be caught
            {
                "workflow": {
                    ...
                    "version": "1.0.0",
                    "inputs": {"file_path": {"type": "string"}, "max_attempts": {"type": "number"}},
                    "default_state": {},
                    "steps": [
                        {
                            "id": "step1",
            ...
            # Completely undefined root-level state field
            {
                "workflow": {
                    ...
                    "steps": [
                        {"id": "step1", "type": "user_message", "message": "Value: {{ state.nonexistent_field }}"}
                    ],
                },
        ...
        for i, test_case in enumerate(test_cases):
            ...
            assert (
                test_case["expected_undefined"] in error_msg
            ), f"Test case {i}: Expected undefined reference '{test_case['expected_undefined']}' not found in: {error_msg}"

            # Check suggestion is provided
            assert "Did you mean" in error_msg, f"Test case {i}: No suggestion provided in error: {error_msg}"
            assert (
                test_case["expected_suggestion"] in error_msg
            ), f"Test case {i}: Expected suggestion '{test_case['expected_suggestion']}' not found in: {error_msg}"
    ...
    def test_malformed_workflow_handling(self):
        ...
        test_cases = [
            ...
            # Invalid nested structures
            {
                ...
                "workflow": {
                    ...
                    "state_schema": {
                        "computed": {"invalid_computed": {"missing_from": True}}  # Missing required 'from' field
                    },
                    "steps": [],
                },
        ...
        for test_case in test_cases:
            ...
            for pattern in test_case["expected_patterns"]:
                assert (
                    pattern.lower() in error_text
                ), f"{test_case['name']}: Expected pattern '{pattern}' not found in errors: {validator.errors}"
    ...
    def test_memory_usage_with_large_workflows(self):
        ...
        for i in range(200):
            ...
            step = {"id": f"step_{i}", "type": step_type}
            ...
            if step_type == "user_message":
                ...
                step["state_update"] = {"path": f"state.result_{i % 10}", "value": f"result_{i}"}
        ...
        ), f"Memory usage too high: {memory_increase / 1024 / 1024:.2f}MB for 200 steps"
    ...
    def test_circular_dependency_detection_in_workflows(self):
        ...
        test_cases = [
            ...
            # Self-referencing computed field
            {
                ...
                "workflow": {
                    ...
                    "version": "1.0.0",
                    "state_schema": {"computed": {"recursive": {"from": "computed.recursive", "transform": "input"}}},
                    "steps": [],
                },
                "should_detect_circular": True,
            },
        ...
        for test_case in test_cases:
            ...
            if test_case["should_detect_circular"]:
                ...
                error_text = " ".join(validator.errors).lower()
                assert (
                    "circular" in error_text or "cycle" in error_text or "dependency" in error_text
                ), f"{test_case['name']}: Should report circular dependency in: {validator.errors}"
            else:
                ...
                ), f"{test_case['name']}: Should not report circular dependency: {circular_errors}"
        ...
        test_cases = [
            ...
            # Very deeply nested template expressions (but valid since level1 exists as object)
            {
                ...
                "workflow": {
                    ...
                    "version": "1.0.0",
                    "default_state": {"level1": {"level2": {"value": 42}}},
                    "steps": [
                        {
                            "id": "step1",
                            "type": "user_message",
                ...
                if test_case.get("should_not_crash"):
                    ...
                    if "expected_patterns" in test_case:
                        ...
                        for pattern in test_case["expected_patterns"]:
                            assert (
                                pattern.lower() in error_text
                            ), f"{test_case['name']}: Expected pattern '{pattern}' not found in: {validator.errors}"
    ...
    def test_helpful_error_message_quality(self):
        ...
        test_cases = [
            ...
            # Test invalid step type
            {
                ...
                "workflow": {
                    ...
                    "version": "1.0.0",
                    "steps": [{"id": "step1", "type": "nonexistent_step_type", "message": "Hello"}],
                },
        ...
        for test_case in test_cases:
            ...
            for guidance in test_case["expected_guidance"]:
                assert (
                    guidance in full_error
                ), f"{test_case['name']}: Expected guidance '{guidance}' not found in error message: {full_error}"
            ...
            assert any(
                keyword in full_error for keyword in ["step", "At", "in", "field", "Config", "Computed"]
            ), f"{test_case['name']}: Error should provide some context: {full_error}"
    ...
    def test_schema_validation_completeness(self):
        ...
        schema_violation_cases = [
            ...
            # Invalid step types
            {
                ...
                "workflow": {
                    ...
                    "version": "1.0.0",
                    "steps": [{"id": "invalid_step", "type": "nonexistent_step_type"}],
                },
        ...
        for test_case in schema_violation_cases:
            ...
            if isinstance(test_case["expected_violation"], list):
                for violation in test_case["expected_violation"]:
                    assert (
                        violation in error_text
                    ), f"{test_case['name']}: Should detect violation '{violation}' in: {validator.errors}"
            else:
                assert (
                    test_case["expected_violation"] in error_text
                ), f"{test_case['name']}: Should detect violation '{test_case['expected_violation']}' in: {validator.errors}"
        ...
        for test_case in warning_cases:
            ...
            assert (
                test_case["expected_warning"] in warning_text
            ), f"{test_case['name']}: Should detect warning '{test_case['expected_warning']}' in: {validator.warnings}"
        ...
        complete_workflow = {
            ...
            "config": {"timeout_seconds": 300, "max_retries": 5},
            "default_state": {"processed_files": [], "error_count": 0, "status": "pending"},
            "state_schema": {
                "computed": {
                    "progress_percent": {
            ...
            "sub_agent_tasks": {
                "file_processor": {
                    ...
                    "steps": [{"id": "process", "type": "shell_command", "command": "process {{ inputs.file }}"}],
                }
            },
        }
        ...
        structural_errors = [e for e in validator.errors if "undefined" not in e.lower()]
