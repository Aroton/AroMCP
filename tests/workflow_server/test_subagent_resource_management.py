...
class TestSubAgentResourceManagement:
    ...
    def test_subagent_timeout_handling_graceful(self, subagent_manager):
        ...
        task_definitions = {
            "fast_task": {
                ...
                "steps": [
                    {"type": "shell_command", "command": "echo 'fast'"}
                ]
            },
            "slow_task": {
                ...
                "inputs": {"timeout_seconds": 2},
                "steps": [
                    {"type": "shell_command", "command": "sleep 5"}  # Exceeds timeout
                ]
            }
        }
    ...
    def test_subagent_status_tracking_monitoring(self, subagent_manager):
        ...
        task_definition = {
            "process_item": {
                ...
                "steps": [
                    {"type": "user_message", "message": "Processing item", "state_update": {"path": "this.status", "value": "processing"}},
                    {"type": "shell_command", "command": "echo 'processing {{ inputs.item_data }}'"},
                    {"type": "user_message", "message": "Processing completed", "state_update": {"path": "this.status", "value": "completed"}}
                ]
            }
        }
    ...
    def test_subagent_failure_isolation(self, subagent_manager):
        ...
        task_definition = {
            "risky_task": {
                ...
                "steps": [
                    {"type": "conditional", "condition": "inputs.should_fail", "then_steps": [
                        {"type": "shell_command", "command": "exit 1"}  # Will fail
                    ], "else_steps": [
                        {"type": "shell_command", "command": "echo 'success'"}
                    ]}
                ]
            }
        }
    ...
    def test_concurrent_subagent_resource_management(self, subagent_manager):
        ...
        work_items = [
            {"id": f"item{i}", "task": "concurrent_task", "data": f"data{i}"}
            for i in range(20)
        ]
        ...
        task_definition = {
            "concurrent_task": {
                ...
                "inputs": {"data": "string"},
                "steps": [
                    {"type": "user_message", "message": "Processing data", "state_update": {"path": "this.processed", "value": "{{ inputs.data }}"}},
        ...
        results = subagent_manager.execute_parallel_tasks(
            work_items, 
            task_definition,
            max_parallel=5  # Limit to 5 concurrent sub-agents
        )
        total_time = time.time() - start_time
    ...
    def test_subagent_memory_management(self, subagent_manager):
        ...
        for i in range(50):
            ...
            large_data = {
                ...
                "nested_data": {
                    "level1": {"level2": {"level3": [f"data{j}" for j in range(100)]}}
                },
        ...
        task_definition = {
            "memory_task": {
                ...
                "steps": [
                    {"type": "user_message", "message": "Calculating size", "state_update": {"path": "this.processed_size", "value": "{{ inputs.large_payload.length }}"}},
                    {"type": "user_message", "message": "Calculating array length", "state_update": {"path": "this.array_length", "value": "{{ inputs.array_data.length }}"}}
                ]
            }
        }
...
class TestSubAgentResourceIntegration:
    ...
    def test_production_parallel_processing_resource_management(self):
        ...
        task_definition = {
            "quality_check": {
                ...
                "steps": [
                    ...
                    {"type": "user_message", "message": "Quality analysis complete", "state_update": {"path": "this.quality_score", "value": "95"}}
                ]
            }
        }
        ...
        results = subagent_manager.execute_parallel_tasks(
            work_items,
            task_definition,
            max_parallel=3,  # Typical production concurrency limit
            timeout_seconds=30
        )
