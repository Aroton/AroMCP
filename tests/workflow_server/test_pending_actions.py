...
class TestPendingActionsManager:
    ...
    def test_basic_operations(self):
        ...
        # Test add
        result = self.manager.add_action(action)
        ...
        # Test remove
        removed = self.manager.remove_action("wf-1")
        assert removed is not None
        assert removed.workflow_id == "wf-1"
    ...
    def test_expiration_cleanup(self):
        ...
        assert "wf-1" in cleaned
        assert len(self.manager._actions) == 0

    def test_list_operations(self):
        """Test list_actions operation."""
        ...
        # Test filtered list
        filtered = self.manager.list_actions(["wf-1", "wf-2"])
    ...
    def test_thread_safety(self):
        ...
        assert len(self.manager._actions) == 5  # Limited by max_capacity
    ...
    def test_clear_all(self):
        ...
        assert len(self.manager._actions) == 3
    ...
    def test_duplicate_workflow_id(self):
        """Test that duplicate workflow IDs are rejected."""
        ...
        action2 = PendingAction(
            workflow_id="wf-1",  # Same workflow ID
            step_id="step-2",
            action_type="shell",
        ...
        result1 = self.manager.add_action(action1)
        assert result1 is True
        
        # Second add should fail
        result2 = self.manager.add_action(action2)
        assert result2 is False
        
        # Should still have only the first action
        assert len(self.manager._actions) == 1
        retrieved = self.manager.get_action("wf-1")
        assert retrieved.step_id == "step-1"
...
class TestGlobalPendingActionsManager:
    ...
    def test_global_manager_functionality(self):
        ...
        result = manager.add_action(action)
        assert result is True
        
        retrieved = manager.get_action("global-test")
        assert retrieved is not None
        assert retrieved.workflow_id == "global-test"
        
        # Clean up
        manager.remove_action("global-test")