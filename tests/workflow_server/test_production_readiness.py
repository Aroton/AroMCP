...
class TestHealthCheckEndpoints:
    ...
    def production_system(self):
        ...
        executor = QueueBasedWorkflowExecutor(
            state_manager=state_manager, observability_manager=observability, error_handler=error_handler
        )

        # Create health check manager
        from aromcp.workflow_server.deployment.health import HealthCheckManager

        health_manager = HealthCheckManager(executor=executor, state_manager=state_manager, observability=observability)
    ...
    async def test_health_check_under_load(self, production_system):
        ...
        simple_workflow = WorkflowDefinition(
            name="load_test", version="1.0", steps=[WorkflowStep(id="wait", type="wait", config={"duration": 1})]
        )
    ...
    async def test_liveness_probe(self, production_system):
        ...
        workflow_def = WorkflowDefinition(
            name="liveness_test",
            version="1.0",
            steps=[WorkflowStep(id="process", type="state_update", config={"updates": {"processed": True}})],
        )
...
class TestGracefulShutdown:
    ...
    async def test_shutdown_with_active_workflows(self):
        ...
        long_workflow = WorkflowDefinition(
            ...
            steps=[
                WorkflowStep(
                    ...
                    config={
                        "items": "{{ range(10) }}",
                        "steps": [{"id": "wait", "type": "wait", "config": {"duration": 0.5}}],
                    },
                )
            ],
        )
        ...
        execution_task = asyncio.create_task(self._execute_workflows(executor))
    ...
    async def test_shutdown_signal_handling(self):
        ...
        try:
            ...
            workflow_def = WorkflowDefinition(
                name="signal_test",
                version="1.0",
                steps=[WorkflowStep(id="process", type="wait", config={"duration": 2})],
            )
...
class TestConfigurationValidation:
    ...
    async def test_configuration_schema_validation(self):
        ...
        config = {
            "workflow_server": {
                ...
                "monitoring": {"metrics_interval": 60, "retention_days": 7, "export_format": "prometheus"},
                "security": {"enable_auth": True, "auth_provider": "jwt", "token_expiry": 3600},
                "database": {"type": "postgresql", "host": "localhost", "port": 5432, "name": "workflow_db"},
            }
        }
        ...
        invalid_config = {
            "workflow_server": {
                ...
                "monitoring": {"export_format": "invalid_format"},  # Invalid format
            }
        }
    ...
    async def test_environment_variable_override(self):
        ...
        try:
            ...
            base_config = {
                "workflow_server": {"max_concurrent_workflows": 10, "default_timeout": 300, "enable_monitoring": True}
            }
    ...
    async def test_configuration_hot_reload(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
            initial_config = {"workflow_server": {"max_concurrent_workflows": 5, "enable_debug": False}}
        ...
        try:
            ...
            # Update config file
            updated_config = {"workflow_server": {"max_concurrent_workflows": 10, "enable_debug": True}}
...
class TestMonitoringAPIAvailability:
    ...
    def metrics_system(self):
        ...
        return {"collector": metrics_collector, "exporter": metrics_exporter, "observability": observability}
    ...
    async def test_metrics_api_endpoints(self, metrics_system):
        ...
        workflow_metrics = await observability.get_workflow_metrics(time_range="1h", aggregation="5m")
        ...
        custom_query = await observability.query_metrics(
            metric="workflow_duration_seconds", labels={"type": "test"}, time_range="30m"
        )
...
class TestAuditTrailCompleteness:
    ...
    async def test_workflow_audit_trail(self):
        ...
        executor = QueueBasedWorkflowExecutor(state_manager=state_manager, audit_logger=audit_logger)

        # Workflow with various operations
        workflow_def = WorkflowDefinition(
            name="audit_test",
            version="1.0",
            metadata={"compliance_level": "high", "data_classification": "sensitive"},
            steps=[
                WorkflowStep(
                    id="user_input",
                    type="user_input",
                    config={"prompt": "Enter sensitive data", "schema": {"type": "object"}},
                ),
                WorkflowStep(
                    id="process_data", type="agent_prompt", config={"prompt": "Process sensitive information"}
                ),
                WorkflowStep(id="store_result", type="state_update", config={"updates": {"processed": True}}),
            ],
        )

        # Execute workflow
        result = await executor.start_workflow(workflow_def, {}, user_context={"user_id": "test_user", "role": "admin"})
...
class TestSecurityBoundaryValidation:
    ...
    async def test_workflow_isolation(self):
        ...
        executor = QueueBasedWorkflowExecutor(state_manager=state_manager, security_manager=security_manager)

        # Create workflows with different security contexts
        workflow_def = WorkflowDefinition(
            name="isolated_workflow",
            version="1.0",
            steps=[WorkflowStep(id="access_data", type="state_update", config={"updates": {"data": "sensitive"}})],
        )
        ...
        with pytest.raises(PermissionError):
            state_manager.get_workflow_state(workflow_id2, security_context=context1)

        # Each tenant can access their own workflow
        state1 = state_manager.get_workflow_state(workflow_id1, security_context=context1)
        assert state1 is not None

        state2 = state_manager.get_workflow_state(workflow_id2, security_context=context2)
    ...
    async def test_resource_access_control(self):
        ...
        policies = {
            ...
            "monitoring": {"read": ["admin", "operator"], "export": ["admin"]},
        }
    ...
    async def test_api_authentication(self):
        ...
        auth_manager = AuthenticationManager(provider="jwt", secret_key="test_secret_key")

        # Generate token for user
        user_info = {"user_id": "test_user", "role": "developer", "tenant_id": "tenant1"}
        ...
        # Test expired token
        expired_token = auth_manager.generate_token(user_info, expiry_seconds=0)  # Already expired
        await asyncio.sleep(0.1)
...
class TestProductionDeploymentScenarios:
    ...
    async def test_rolling_deployment_compatibility(self):
        ...
        old_workflow = {
            ...
            "version": "1.0",
            "api_version": "v1",
            "steps": [{"id": "step1", "type": "state_update", "config": {"updates": {"value": "test"}}}],
        }
            ...
            "steps": [
                {
                    "id": "step1",
                    "type": "state_update",
                    "config": {"updates": {"value": "test"}, "validation": {"required": True}},  # New feature
                }
            ],
        }

        # Test backward compatibility
        is_compatible = version_manager.check_compatibility(old_version=old_workflow, new_version=new_workflow)

        assert is_compatible == True

        # Test migration path
        migration_plan = version_manager.get_migration_plan(from_version="v1", to_version="v2")
    ...
    async def test_backup_and_recovery(self):
        ...
        workflow_def = WorkflowDefinition(
            name="backup_test",
            version="1.0",
            steps=[WorkflowStep(id="step1", type="state_update", config={"updates": {"data": "important"}})],
        )
        ...
        # Create backup
        backup_id = await backup_manager.create_backup(backup_type="full", description="Test backup")
        ...
        # Test restore
        restore_result = await backup_manager.restore_backup(backup_id=backup_id, target="test_restore")
