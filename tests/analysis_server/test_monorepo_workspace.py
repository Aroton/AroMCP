...
class TestMonorepoWorkspaceDiscovery:
    ...
    def test_cross_project_symbol_resolution(self, simple_monorepo):
        ...
        references = workspace_context.find_references(
            symbol_name="CoreService",
            project="@monorepo/web"
        )
    ...
    def test_isolated_project_analysis(self, simple_monorepo):
        ...
        core_result = analyzer.analyze_project(
            project_name="@monorepo/core",
            isolated=True
        )
    ...
    def test_shared_context_analysis(self, simple_monorepo):
        ...
        ui_result = analyzer.analyze_project(
            project_name="@monorepo/ui",
            isolated=False
        )
        ...
        workspace_unresolved = [
            imp for imp in ui_result.unresolved_imports 
...
class TestMonorepoPerformance:
    ...
    def large_monorepo(self, tmp_path):
        ...
        root_tsconfig = {
            ...
            "references": [
                {"path": f"./packages/pkg-{i}"} for i in range(num_packages)
            ]
        }
        (tmp_path / "tsconfig.json").write_text(json.dumps(root_tsconfig, indent=2))
        ...
        for i in range(num_packages):
            ...
            if i > 0:
                dependencies.extend([
                    {"path": f"../pkg-{j}"} for j in range(max(0, i-3), i)
                ])
            ...
            for j in range(10):  # 10 files per package
                (pkg_dir / "src" / f"module{j}.ts").write_text(f"""
    ...
    def test_parallel_project_analysis(self, large_monorepo):
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            par_results = list(executor.map(
                lambda p: analyzer.analyze_project(p.name, isolated=True),
                projects
            ))
