...
"""
    ...
    def test_detect_modified_files(self, project_with_files):
        ...
        # Get initial state
        initial_hashes = {str(f): tracker.get_file_metadata(str(f)).content_hash for f in files.values()}
...
"""
    ...
    def test_70_percent_time_reduction(self, large_project):
        ...
        assert (
            no_change_result.files_analyzed <= len(files) * 0.3
        ), f"No changes: analyzed {no_change_result.files_analyzed} files"
        ...
        root_files = [
            f for f in files if "module_0" in str(f) or "module_01" in str(f)
        ]  # Early modules affect many others
        if root_files:
    ...
    def test_smart_dependency_reanalysis(self, large_project):
        ...
        leaf_files = [f for f in files if len(dep_graph.get_dependents(str(f))) == 0]
        ...
        root_files = [f for f in files if len(dep_graph.get_dependencies(str(f))) == 0]
