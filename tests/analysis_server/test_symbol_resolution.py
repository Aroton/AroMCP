...
describe('AuthService', () => {
    ...
    def test_syntactic_symbol_resolution(self, resolver, phase2_project):
        ...
        method_references = [
            ...
            if ref.reference_type == ReferenceType.DEFINITION and ref.symbol_type == SymbolType.METHOD
        ]
    ...
    def test_semantic_symbol_resolution(self, resolver, phase2_project):
        ...
        import_references = [ref for ref in result.references if ref.reference_type == ReferenceType.IMPORT]
        ...
        implements_refs = [
            ref
            for ref in result.references
            if ref.symbol_name == "User" and ref.reference_type == ReferenceType.USAGE and "implements" in ref.context
        ]
    ...
    def test_dynamic_symbol_resolution_with_inheritance(self, resolver, phase2_project):
        ...
        for chain in result.inheritance_chains:
            if chain.base_class == "BaseUser" and "AuthenticatedUser" in chain.derived_classes:
        ...
        get_display_name_refs = [ref for ref in result.references if ref.symbol_name == "getDisplayName"]

        # Should find both abstract definition and concrete implementations
        declarations = [ref for ref in get_display_name_refs if ref.reference_type == ReferenceType.DECLARATION]
        definitions = [ref for ref in get_display_name_refs if ref.reference_type == ReferenceType.DEFINITION]
    ...
    def test_class_method_syntax_resolution(self, resolver, phase2_project):
        ...
        method_calls = [
            ref
            for ref in result.references
            if ref.symbol_name == "authenticate" and ref.reference_type == ReferenceType.CALL
        ]
    ...
    def test_exclude_tests_by_default(self, resolver, phase2_project):
        ...
        test_symbols = [name for name, info in result.symbols.items() if info.file_path.endswith("auth.test.ts")]
        assert len(test_symbols) == 0

        # Should not include references from test files
        test_references = [ref for ref in result.references if ref.file_path.endswith("auth.test.ts")]
    ...
    def test_include_tests_when_requested(self, resolver, phase2_project):
        ...
        all_files = [str(phase2_project / "src" / "auth" / "user.ts"), str(phase2_project / "tests" / "auth.test.ts")]
        ...
        test_function_calls = [
            ref
            for ref in result.references
            if ref.symbol_name in ["describe", "test", "expect"] and ref.file_path.endswith("auth.test.ts")
        ]
    ...
    def test_error_handling_and_recovery(self, resolver, phase2_project):
        ...
        valid_symbols = [
            info for info in result.symbols.values() if not info.file_path.endswith("/nonexistent/file.ts")
        ]
