...
class TestFocusedAdvancedTypeResolution:
    ...
    def test_specific_error_codes_implementation(self, temp_project):
        ...
        assert (
            "UNKNOWN_TYPE" in error_codes
            or "TYPE_RESOLUTION_ERROR" in error_codes
            or "CIRCULAR_REFERENCE_DETECTED" in error_codes
        ), f"Expected UNKNOWN_TYPE, TYPE_RESOLUTION_ERROR, or CIRCULAR_REFERENCE_DETECTED in {error_codes}"
        assert (
            "CIRCULAR_REFERENCE_DETECTED" in error_codes or "TYPE_RESOLUTION_ERROR" in error_codes
        ), f"Expected CIRCULAR_REFERENCE_DETECTED in {error_codes}"
    ...
    def test_max_constraint_depth_parameter_enforcement(self, temp_project):
        ...
        ), f"Expected CONSTRAINT_DEPTH_EXCEEDED in {depth_error_codes}"
    ...
    def test_track_instantiations_parameter_implementation(self, temp_project):
        ...
        assert (
            len(container_instantiations) >= 3
        ), f"Expected at least 3 Container instantiations, got {len(container_instantiations)}"

        # Should track Processor instantiations
        processor_instantiations = result.type_instantiations.get("Processor", [])
        assert (
            len(processor_instantiations) >= 3
        ), f"Expected at least 3 Processor instantiations, got {len(processor_instantiations)}"
    ...
    def test_resolve_conditional_types_parameter(self, temp_project):
        ...
        assert (
            "ComplexConditional<T>" in test_func.signature
        ), f"Expected ComplexConditional<T> in signature: {test_func.signature}"
        ...
        assert (
            "ExtractArrayType<T>" in test_func.signature
        ), f"Expected ExtractArrayType<T> in signature: {test_func.signature}"
        ...
        for cond_type in conditional_types:
            assert (
                cond_type in test_func.types
            ), f"Expected {cond_type} in function types: {list(test_func.types.keys())}"
    ...
    def test_fallback_on_complexity_metadata_tracking(self, temp_project):
        ...
        assert isinstance(
            result.resolution_metadata, TypeResolutionMetadata
        ), "resolution_metadata should be TypeResolutionMetadata instance"
        ...
        ), f"Expected fallback usage > 0, got {result.resolution_metadata.fallbacks_used}"

        # Should indicate resolution depth used
        assert result.resolution_metadata.resolution_depth_used in [
            "basic",
            "generics",
        ], f"Unexpected resolution depth: {result.resolution_metadata.resolution_depth_used}"
    ...
    def test_error_categorization_in_complex_scenarios(self, temp_project):
        ...
        if result.errors:
            ...
            assert any(
                cat in expected_categories for cat in error_categories
            ), f"Expected specific error categories, got: {error_categories}"
            ...
            if circular_errors:
                assert any(
                    e.code == "CIRCULAR_REFERENCE_DETECTED" for e in circular_errors
                ), "Expected CIRCULAR_REFERENCE_DETECTED for circular references"
    ...
    def test_constraint_depth_metadata_tracking(self, temp_project):
        ...
        ), "Missing max_constraint_depth_reached in metadata"
        ...
        if analysis_time > 5.0 or result.errors:
            ...
            if result.errors:
                ...
                if not has_performance_error:
                    ...
                    assert (
                        "TYPE_RESOLUTION_ERROR" in error_codes
                    ), f"Expected performance-related error codes, got: {error_codes}"
