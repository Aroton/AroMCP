...
describe('UserService', () => {
    ...
    def test_find_references_class_method_syntax(self, temp_project):
        ...
        for ref in result.references:
            if ref.symbol_name == "hasPermission" and ref.class_name == "AdminUser":
    ...
    def test_find_references_exclude_tests_by_default(self, temp_project):
        ...
        result = find_references_impl(
            symbol="AdminUser", file_paths=all_files, include_declarations=True, include_usages=True
        )
    ...
    def test_find_references_single_symbol(self, temp_project):
        ...
        result = find_references_impl(
            symbol="AdminUser", file_paths=str(admin_user_file), include_declarations=True, include_usages=True
        )
    ...
    def test_find_references_multiple_files(self, temp_project):
        ...
        result = find_references_impl(
            symbol="User", file_paths=file_paths, include_declarations=True, include_usages=True
        )
    ...
    def test_find_references_with_filters(self, temp_project):
        ...
        for filters in filter_scenarios:
            result = find_references_impl(symbol="TestInterface", file_paths=str(test_file), **filters)
...
class TestGetFunctionDetailsTool:
    ...
    def test_get_function_details_error_scenarios(self, temp_project):
        ...
        assert (
            ...
            or all(len(func_list) == 0 for func_list in result.functions.values())
        )
    ...
    def test_get_function_details_filtering_options(self, temp_project):
        ...
        for options in option_combinations:
            result = get_function_details_impl(functions="simpleFunction", file_paths=str(test_file), **options)
...
class TestGetCallTraceTool:
    ...
    def test_get_call_trace_simple_flow(self, temp_project):
        ...
        result = get_call_trace_impl(
            entry_point="main", file_paths=str(test_file), max_depth=5, include_external_calls=False
        )
    ...
    def test_get_call_trace_cross_file_analysis(self, temp_project):
        ...
        result = get_call_trace_impl(
            entry_point="UserController.handleGetUser", file_paths=file_paths, max_depth=10, include_external_calls=True
        )
    ...
    def test_get_call_trace_error_handling(self, temp_project):
        ...
        result = get_call_trace_impl(
            entry_point="nonExistentFunction", file_paths=str(test_file), max_depth=5, include_external_calls=False
        )
    ...
    def test_get_call_trace_statistics_accuracy(self, temp_project):
        ...
        result = get_call_trace_impl(
            entry_point="main", file_paths=str(test_file), max_depth=10, include_external_calls=False
        )
    ...
    def test_phase4_cycle_detection_without_infinite_loops(self, phase4_fixtures_dir):
        ...
        result = get_call_trace_impl(
            entry_point="factorial", file_paths=[recursive_file], max_depth=20, analyze_conditions=True
        )
    ...
    def test_phase4_conditional_execution_path_analysis(self, phase4_fixtures_dir):
        ...
        result = get_call_trace_impl(
            entry_point="processUser", file_paths=[conditional_file], max_depth=12, analyze_conditions=True
        )
    ...
    def test_phase4_class_method_inheritance_tracing(self, phase4_fixtures_dir):
        ...
        method_calls = {"validateUserData", "normalizeUserData", "findUserByEmail", "hashPassword", "sendWelcomeEmail"}
        ...
        result_inheritance = get_call_trace_impl(
            entry_point="save", file_paths=[class_methods], max_depth=8  # BaseEntity.save (inherited)
        )
    ...
    def test_phase4_error_handling_and_edge_cases(self, phase4_fixtures_dir):
        ...
        result_invalid = get_call_trace_impl(entry_point="nonExistentFunction", file_paths=[simple_calls], max_depth=5)
        ...
        result_empty = get_call_trace_impl(entry_point="", file_paths=[simple_calls], max_depth=5)
        ...
        result_deep = get_call_trace_impl(
            entry_point="login", file_paths=[simple_calls], max_depth=100  # Very high depth
        )
...
class TestToolIntegration:
    ...
    def test_tool_chain_analysis_workflow(self, temp_project):
        ...
        references_result = find_references_impl(
            symbol="User", file_paths=str(test_file), include_declarations=True, include_usages=True
        )
        ...
        call_trace_result = get_call_trace_impl(
            entry_point="UserService.getUser", file_paths=str(test_file), max_depth=5, include_external_calls=True
        )
