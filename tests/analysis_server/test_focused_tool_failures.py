...
class TestFindReferencesSpecificFailures:
    ...
    def test_include_tests_false_should_filter_test_files_by_default(self, temp_project):
        ...
        test_references = [ref for ref in result.references if ref.file_path.endswith(".test.ts")]

        # THIS SHOULD PASS BUT CURRENTLY FAILS:
        assert (
            len(test_references) == 0
        ), f"Found {len(test_references)} references in test files, but include_tests=False should exclude them"

        # Should still find references from source files
        source_references = [ref for ref in result.references if not ref.file_path.endswith(".test.ts")]
    ...
    def test_include_tests_true_should_include_test_files(self, temp_project):
        ...
        assert (
            len(test_references) > 0
        ), f"Expected references in test files with include_tests=True, but found {len(test_references)}"
    ...
    def test_method_syntax_detection_for_class_methods_failing(self, temp_project):
        ...
        validate_user_refs = [ref for ref in result.references if ref.symbol_name == "validateUser"]
        ...
        for ref in validate_user_refs:
            ...
            ), "Reference should have class_name attribute for method syntax, but found None"
            assert class_name == "UserService", f"Expected class_name='UserService', got '{class_name}'"
            assert (
                method_name is not None
            ), "Reference should have method_name attribute for method syntax, but found None"
...
class TestGetFunctionDetailsSpecificFailures:
    ...
    def test_imported_type_resolution_not_working_properly(self, temp_project):
        ...
        if process_user and process_user.types:
            ...
            assert (
                "User" in process_user.types
            ), f"Should find User type, found types: {list(process_user.types.keys())}"
            assert (
                "BaseEntity" in process_user.types
            ), f"Should find BaseEntity type (imported), found types: {list(process_user.types.keys())}"
        ...
        if validate_entity_list:
            ...
            if validate_entity:
                ...
                assert (
                    "T extends BaseEntity" in validate_entity.signature
                ), "Should show generic constraint with BaseEntity"

                if validate_entity.types:
                    # THIS SHOULD PASS BUT CURRENTLY FAILS:
                    assert (
                        "BaseEntity" in validate_entity.types
                    ), f"Should resolve BaseEntity in generic function, found types: {list(validate_entity.types.keys())}"
...
class TestGetCallTraceSpecificFailures:
    ...
    def test_performance_constraint_issue_max_depth_exceeded(self, temp_project):
        ...
        assert (
            result.call_graph_stats.max_depth_reached <= 5
        ), f"max_depth_reached is {result.call_graph_stats.max_depth_reached}, should be ≤ 5"
    ...
    def test_error_handling_not_working_for_non_existent_entry_points(self, temp_project):
        ...
        assert (
            len(result.errors) > 0
        ), f"Should report error for non-existent entry point, but errors array is empty: {result.errors}"
        ...
        assert error.code in [
            "NOT_FOUND",
            "INVALID_ENTRY_POINT",
        ], f"Error code should be NOT_FOUND or INVALID_ENTRY_POINT, got '{error.code}'"
        assert "nonExistentFunction" in error.message, "Error message should mention the non-existent function name"
    ...
    def test_error_handling_empty_entry_point_should_report_error(self, temp_project):
        ...
        ), f"Should have INVALID_ENTRY_POINT error, found error codes: {[e.code for e in result.errors]}"

        validation_error = validation_errors[0]
        assert (
            "empty" in validation_error.message.lower() or "invalid" in validation_error.message.lower()
    ...
    def test_depth_limit_enforcement_with_complex_recursion(self, temp_project):
        ...
        assert (
            result.call_graph_stats.max_depth_reached <= 3
...
class TestAdditionalEdgeCaseFailures:
    ...
    def test_find_references_should_fail_with_test_files_when_include_tests_false(self, temp_project):
        ...
        result = find_references_impl(
            symbol="TestUser", file_paths=all_files, include_declarations=True, include_usages=True, include_tests=False
        )
    ...
    def test_get_call_trace_should_handle_malformed_functions_gracefully(self, temp_project):
        ...
        result = get_call_trace_impl(
            entry_point="validFunction", file_paths=str(malformed_file), max_depth=5, include_external_calls=False
        )
            ...
            try:
                ...
                if hasattr(result, "errors") and len(result.errors) > 0:
                    ...
                    param_errors = [
                        ...
                        if "parameter" in e.message.lower()
                    ...
                    assert (
                        len(param_errors) > 0
                    ), f"Test case {i}: Should report parameter validation errors for unsupported parameters"
                else:
                    ...
                    assert (
                        False
                    ), f"Test case {i}: Implementation should validate parameters and report errors for unsupported options"

            except TypeError as e:
                # This is expected for truly unsupported parameters
                assert "unexpected keyword argument" in str(e) or "argument" in str(
                    e
                ), f"Test case {i}: Should get meaningful TypeError for unsupported parameters"
