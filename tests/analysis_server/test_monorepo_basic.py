...
def basic_monorepo(tmp_path):
    ...
    (shared_dir / "tsconfig.json").write_text(json.dumps({
        "compilerOptions": {
            "composite": True,
            "declaration": True,
            "outDir": "./dist"
...
""")
    ...
    (app_dir / "tsconfig.json").write_text(json.dumps({
        "compilerOptions": {
            "composite": True
        },
        "references": [
...
class TestBasicMonorepoFunctionality:
    ...
    def test_workspace_context_includes_dependencies(self, basic_monorepo):
        ...
        workspace_unresolved = [
            imp for imp in result.unresolved_imports 
            if imp.startswith("@test/")
        ]
