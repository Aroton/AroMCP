...
class TestAdvancedTypeResolutionFailures:
    ...
    def test_resolution_depth_content_differences(self, temp_project):
        ...
        basic_result = get_function_details_impl(
            functions="processComplex", file_paths=str(test_file), include_types=True, resolution_depth="basic"
        )

        generic_result = get_function_details_impl(
            functions="processComplex", file_paths=str(test_file), include_types=True, resolution_depth="generics"
        )
    ...
    def test_type_resolution_error_handling(self, temp_project):
        ...
        assert any(
            code in expected_codes for code in error_codes
        ), f"Expected error codes {expected_codes}, got {error_codes}"
    ...
    def test_type_resolution_depth_fallback(self, temp_project):
        ...
        assert any(
            e.code == "CONSTRAINT_DEPTH_EXCEEDED" for e in depth_errors
        ), "Expected CONSTRAINT_DEPTH_EXCEEDED error code"
