    ...
    async def test_analysis_tools_registration(self, mcp_server):
        ...
        for tool_name in expected_tools:
            assert tool_name in tool_names, (
                f"Tool {tool_name} not properly registered with FastMCP. "
...
class TestJsonConvertDecorator:
    ...
    def test_json_convert_with_complex_nested_parameters(self, temp_project):
        ...
        filters = [
            {"pattern": "*.ts", "exclude": "test"},
            {"pattern": "*.tsx", "exclude": "node_modules"}
        ]
    ...
    def test_union_type_runtime_behavior(self):
        ...
        def mock_analysis_tool(
            ...
            return {
                ...
                "metadata_keys": len(metadata_dict)
            }
        ...
        result = mock_analysis_tool(
            symbols="test",
            metadata=json.dumps({"opt1": "val1", "opt2": "val2"})
        )
...
class TestMCPProtocolCompliance:
    ...
    def test_tool_parameter_validation(self):
        ...
        expected_parameters = {
            "find_references": [
                "symbol", "file_paths", "include_declarations", 
                "include_usages", "page", "max_tokens"
            ],
            "get_function_details": [
                "functions", "file_paths", "include_code", 
        ...
        for tool_name, params in expected_parameters.items():
            for param in params:
                ...
                assert "_" in param or param.islower(), (
                    f"Parameter {param} in {tool_name} should use snake_case"
                )
    ...
    def test_pagination_parameter_consistency(self):
        ...
        pagination_response_fields = [
            "total", "page_size", "next_cursor", "has_more"
        ]
    ...
    def test_tool_namespace_isolation(self):
        ...
        try:
            ...
            for tool_name in analysis_tool_names:
                assert tool_name not in existing_tools, (
                    f"Analysis tool {tool_name} conflicts with existing tool"
                )
