...
try:
    ...
    from aromcp.analysis_server.tools.batch_processor import BatchConfig, BatchProcessor
...
"""
    ...
    def create_large_project(root_path: Path, num_files: int, structure: str = "flat") -> dict[str, Any]:
        ...
        stats = {"total_files": 0, "total_lines": 0, "total_bytes": 0, "directories": []}
...
class TestLargeScalePerformance:
    ...
    def large_project_50k(self, tmp_path_factory):
        ...
        stats = LargeProjectGenerator.create_large_project(
            root, num_files=5000, structure="modular"  # 5k files representing 50k
        )
    ...
    def test_initial_parsing_performance(self, large_project_50k):
        ...
        parser = TypeScriptParser(cache_size_mb=200, max_file_size_mb=10)  # Larger cache for big project
    ...
    def test_memory_usage_50k_files(self, large_project_50k):
        ...
        parser = TypeScriptParser(cache_size_mb=150, max_file_size_mb=5, memory_manager=memory_manager)
    ...
    def test_batch_processing_performance(self, large_project_50k):
        ...
        batch_config = BatchConfig(batch_size=100, max_memory_mb=400, timeout_seconds=10)
        ...
        results = batch_processor.process_files(
            file_paths=[str(f) for f in ts_files], operation="parse", resolution_depth=ResolutionDepth.SYNTACTIC
        )
    ...
    def test_parallel_analysis_scaling(self, large_project_50k):
        ...
        if len(timings) >= 2:
            ...
            assert (
                timings[2] < timings[1] * 1.5
            ), f"2 workers significantly slower than 1: {timings[2]:.2f}s vs {timings[1]:.2f}s"

            # 4 workers should be faster than 2 (if enough cores) - but allow overhead
            if os.cpu_count() and os.cpu_count() >= 4:
                assert (
                    timings[4] < timings[2] * 1.3
                ), f"4 workers much slower than 2: {timings[4]:.2f}s vs {timings[2]:.2f}s"
...
class TestIncrementalAnalysisPerformance:
    ...
    def project_with_history(self, tmp_path):
        ...
        stats = LargeProjectGenerator.create_large_project(tmp_path, num_files=100, structure="modular")
...
"""
        ...
        modified_base = base_content.replace("name: string;", "name: string;\n    description?: string;")
    ...
    def test_incremental_cache_efficiency(self, project_with_history):
        ...
        analyzer = IncrementalAnalyzer(project_root=str(root), cache_size_mb=100)
...
class TestMemoryOptimization:
    ...
    def test_compressed_ast_storage(self, tmp_path):
        ...
        parser_uncompressed = TypeScriptParser(cache_size_mb=100, enable_compression=False)
        ...
        if psutil:
            memory_uncompressed = max(
                0.1, process.memory_info().rss / (1024 * 1024) - memory_before
        ...
        parser_compressed = TypeScriptParser(cache_size_mb=100, enable_compression=True)
        ...
        if psutil:
            ...
            reduction = (
                (memory_uncompressed - memory_compressed) / memory_uncompressed if memory_uncompressed > 0 else 0
            )
            assert (
                reduction >= -0.2
            ), f"Memory usage increased significantly: {reduction:.1%} - compression may not be effective but shouldn't hurt"
        else:
...
"""
        ...
        parser = TypeScriptParser(cache_size_mb=50, enable_string_interning=True)
    ...
    def test_memory_pressure_handling(self, tmp_path):
        ...
        parser = TypeScriptParser(
            cache_size_mb=200, memory_manager=memory_manager  # Use substantial cache with higher memory budget
        )
        ...
        assert (
            memory_pressures_handled > 0 or final_memory <= 400
        ), f"Either memory pressure should trigger or stay under 400MB (got {final_memory}MB)"
