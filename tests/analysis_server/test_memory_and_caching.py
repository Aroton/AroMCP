...
def cache_manager():
    ...
    return CacheManager(levels=[CacheLevel.MEMORY, CacheLevel.SYMBOL, CacheLevel.FILESYSTEM])
...
"""
    ...
    def test_adaptive_cache_sizing(self, project_files):
        ...
        cache_manager = CacheManager(enable_adaptive_sizing=True, min_size_mb=50, max_size_mb=200)
    ...
    def test_cache_compression_tradeoffs(self, project_files):
        ...
        assert (
            results[CompressionStrategy.NONE]["avg_parse_time"]
            <= results[CompressionStrategy.ZLIB]["avg_parse_time"] * 1.5
        )  # Allow some overhead
...
"""
        ...
        # Track dependencies
        dependency_tracker.add_dependency(str(consumer_file), str(base_file), imports=["BaseInterface", "BaseClass"])
    ...
    def test_invalidate_dependent_caches(self, dependency_tracker, cache_manager, tmp_path):
        ...
        invalidation_strategy = CacheInvalidationStrategy(
            dependency_tracker=dependency_tracker, cache_manager=cache_manager
        )
    ...
    def test_selective_invalidation(self, dependency_tracker, cache_manager):
        ...
        dependency_tracker.add_dependency("fileB", "fileA", imports=["funcA", "InterfaceA"])

        # File C imports ClassA
        dependency_tracker.add_dependency("fileC", "fileA", imports=["ClassA"])
        ...
        invalidation_strategy = CacheInvalidationStrategy(
            dependency_tracker=dependency_tracker, cache_manager=cache_manager, enable_selective=True
        )

        invalidated = invalidation_strategy.invalidate_file("fileA", changed_exports=["constA"])
...
class TestCacheStatisticsAndMonitoring:
    ...
    def test_cache_performance_metrics(self):
        ...
        cache_manager = CacheManager(
            enable_performance_metrics=True, levels=[CacheLevel.MEMORY]  # Only use memory cache for speed
        )
    ...
    def test_cache_monitoring_alerts(self):
        ...
        cache_manager = CacheManager(enable_monitoring=True, alert_handler=alert_handler)
