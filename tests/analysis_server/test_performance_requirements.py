...
class TestPerformanceRequirements:
    ...
    def test_parsing_speed_requirement_large_file(self, parser, fixtures_dir):
        ...
        assert median_time_ms <= expected_max_time_ms, (
            f"Large file parsing: {median_time_ms:.2f}ms for {loc} LOC "
            f"exceeds {expected_max_time_ms:.2f}ms limit"
        )
...
"""
            ...
            assert successful_parses >= len(files_created) * 0.95, (
...
"""
            ...
            if pattern_hits + pattern_misses > 0:
                ...
                assert hit_ratio > 0.90, (
                    f"Cache hit ratio {hit_ratio:.2%} too low for realistic workload"
                )
...
"""
    ...
    def test_cache_statistics_accuracy_under_load(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert final_stats.files_parsed == expected_files_parsed, (
            ...
            assert abs(actual_hit_rate - expected_hit_rate) < 0.1, (
                f"Hit rate calculation incorrect: expected {expected_hit_rate:.1f}%, "
                f"got {actual_hit_rate:.1f}%"
            )