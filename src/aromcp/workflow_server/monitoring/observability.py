...
class StatusAPI:
    ...
    def get_workflow_health(self, workflow_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "resource_usage": {"memory_percent": health.memory_usage_percent, "cpu_percent": health.cpu_usage_percent},
...
class ObservabilityManager:
    ...
    def __init__(self):
        ...
        self._alert_handler: Callable | None = None
        ...
        with self._lock:
            for config in self._external_configs:
                ...
                if config["format"] == "prometheus":
                    ...
                    formatted = "\n".join(
                        [f"{key} {value}" for key, value in metrics.items() if key in config.get("metrics", [])]
                    )
    ...
    def generate_audit_trail(self, workflow_id: str) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            if not events:
                return {"workflow_id": workflow_id, "events": [], "total_duration": 0}
    ...
    def configure_alert_channel(self, config: dict[str, Any]):
        ...
        with self._lock:
            alert_config = AlertConfig(
                channel=config["channel"], severity_threshold=config.get("severity_threshold", "medium"), config=config
            )
    ...
    def trigger_failure_alert(
        self, workflow_id: str, error_type: str, severity: str, message: str, context: dict[str, Any] | None = None
    ):
    ...
    def record_system_health(
        self, timestamp: datetime, memory_usage_percent: float, cpu_usage_percent: float, error_rate: float
    ):
    ...
    def evaluate_system_health(self) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            return {"status": status, "metrics": latest}
    ...
    def get_current_system_health(self) -> dict[str, Any]:
        ...
        with self._lock:
            if not self._system_health_history:
                return {"status": "unknown", "memory_usage_percent": 0, "cpu_usage_percent": 0, "error_rate": 0}
