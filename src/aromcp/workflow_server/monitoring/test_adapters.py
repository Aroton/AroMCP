...
class MetricsCollectorTestAdapter:
    ...
    def __init__(self):
        ...
        self._step_starts = {}
        self._step_type_metrics = defaultdict(
            lambda: {"count": 0, "total_duration": 0, "success_count": 0, "failure_count": 0, "timeout_count": 0}
        )

    def record_step_execution(self, step_id: str, step_type: str, duration: float, status: str, timestamp: float):
        """Record a step execution."""
        self._step_executions.append(
            {"step_id": step_id, "step_type": step_type, "duration": duration, "status": status, "timestamp": timestamp}
        )
    ...
    def record_step_start(self, step_id: str, step_type: str, timestamp: float):
        """Record step start."""
        self._step_starts[step_id] = {"step_type": step_type, "start_time": timestamp}
    ...
    def record_step_completion(self, step_id: str, duration: float, status: str, timestamp: float):
        ...
        if step_id in self._step_starts:
            ...
            self.record_step_execution(
                step_id=step_id, step_type=step_info["step_type"], duration=duration, status=status, timestamp=timestamp
            )
...
class ScalingManagerTestAdapter:
    ...
    def register_execution_instance(self, instance_id: str, capacity: int, current_load: int, health_status: str):
