...
class PerformanceMonitor:
    ...
    def __init__(self):
        ...
        self._step_type_stats: dict[str, dict[str, Any]] = defaultdict(lambda: {
            "count": 0,
            "total_duration": 0,
            "total_cpu": 0,
            "total_memory": 0,
    ...
    def record_step_performance(self, step_id: str, step_type: str, duration: float,
    ...
    def record_resource_usage(self, timestamp: datetime, memory_mb: float, 
                            cpu_percent: float, context: dict[str, Any]):
    ...
    def compare_execution_modes(self, mode1: str, mode2: str) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            resources = {
                ...
                "memory_overhead_factor": data1.peak_memory_mb / data2.peak_memory_mb if data2.peak_memory_mb > 0 else 0,
            ...
            # Efficiency analysis
            efficiency = {
                "parallel_efficiency": min(1.0, timing["speedup_factor"] / data1.concurrent_steps) if mode1 == "parallel" else 0,
                "resource_efficiency_score": data1.resource_efficiency,
                "cpu_utilization_improvement": (data1.average_cpu_percent - data2.average_cpu_percent) / data2.average_cpu_percent if data2.average_cpu_percent > 0 else 0,
                "memory_cost_analysis": (data1.peak_memory_mb - data2.peak_memory_mb) / timing.get("time_saved_seconds", 1) if timing.get("time_saved_seconds", 0) != 0 else 0
            }
    ...
    def start_operation(self, operation_id: str, metadata: dict = None) -> str:
        ...
        with self._lock:
            ...
            self._step_data.append(type('Operation', (), {
    ...
    def get_bottleneck_analysis(self) -> dict[str, Any]:
        ...
        sorted_bottlenecks = sorted(bottlenecks, 
                                   key=lambda x: (severity_order.index(x.severity), x.value), 
                                   reverse=True)
    ...
    def get_events(self, event_type: str) -> list[dict[str, Any]]:
        ...
        with self._lock:
            events = []
            for snapshot in self._resource_timeline:
                if (snapshot.context.get("is_event") and 
                    snapshot.context.get("event_type") == event_type):
                    events.append({
