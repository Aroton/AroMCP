...
class AgentPromptProcessor:
    ...
    def process_agent_prompt(self, step: WorkflowStep, state: dict[str, Any]) -> dict[str, Any]:
        ...
        if not prompt:
            return {
                "error": f"Agent prompt step {step.id} missing required 'prompt' field"
            }
    ...
    def validate_agent_response(
        self, step: WorkflowStep, agent_response: dict[str, Any]
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if response_type == "object":
                if not isinstance(agent_response, dict):
                    return {
                        "valid": False,
                        "error": f"Expected object response, got {type(agent_response).__name__}"
                    }
                ...
                for field in required_fields:
                    if field not in agent_response:
                        return {
                            "valid": False,
                            "error": f"Required field '{field}' missing from response"
                        }
                ...
                if not isinstance(agent_response, list):
                    return {
                        "valid": False,
                        "error": f"Expected array response, got {type(agent_response).__name__}"
                    }
                ...
                if not isinstance(agent_response, str):
                    return {
                        "valid": False,
                        "error": f"Expected string response, got {type(agent_response).__name__}"
                    }
                ...
                if not isinstance(agent_response, (int, float)):
                    return {
                        "valid": False,
                        "error": f"Expected number response, got {type(agent_response).__name__}"
                    }
                ...
                if not isinstance(agent_response, bool):
                    return {
                        "valid": False,
                        "error": f"Expected boolean response, got {type(agent_response).__name__}"
                    }
