...
class AgentResponseProcessor:
    ...
    def process_agent_response(
        ...
        for update in state_updates:
            ...
            try:
                ...
                updates_to_apply.append({"path": path, "value": value, "operation": operation})
        ...
        if store_response:
            updates_to_apply.append({"path": store_response, "value": agent_response, "operation": "set"})
    ...
    def _validate_response(
        self, response: dict[str, Any], schema: dict[str, Any] | None, validation_rules: dict[str, Any] | None
    ) -> dict[str, Any]:
        """
    ...
    def _validate_against_schema(self, response: dict[str, Any], schema: dict[str, Any]) -> dict[str, Any]:
        ...
        if response_type == "object":
            if not isinstance(response, dict):
                return {"valid": False, "error": f"Expected object, got {type(response).__name__}"}
            ...
            if not isinstance(response, list):
                return {"valid": False, "error": f"Expected array, got {type(response).__name__}"}
            ...
            if not isinstance(response, expected_types[response_type]):
                return {"valid": False, "error": f"Expected {response_type}, got {type(response).__name__}"}
        ...
        try:
            ...
            if max_length and isinstance(response, (str, list, dict)):
                if len(response) > max_length:
                    return {"valid": False, "error": f"Response length {len(response)} exceeds maximum {max_length}"}

            # Check allowed values
            allowed_values = rules.get("allowed_values")
            if allowed_values and response not in allowed_values:
                return {"valid": False, "error": f"Response value '{response}' not in allowed values: {allowed_values}"}
            ...
            if pattern and isinstance(response, str):
                ...
                if not re.match(pattern, response):
                    return {"valid": False, "error": f"Response does not match required pattern: {pattern}"}
            ...
            if custom_validator and callable(custom_validator):
                try:
                    if not custom_validator(response):
                        return {"valid": False, "error": "Response failed custom validation"}
                except Exception as e:
                    return {"valid": False, "error": f"Custom validation error: {str(e)}"}

            return {"valid": True}

        except Exception as e:
            return {"valid": False, "error": f"Validation rules processing error: {str(e)}"}
