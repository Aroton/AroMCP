...
class DebugManager:
    ...
    def create_checkpoint(
        self, workflow_id: str, step_id: str, state_before: dict[str, Any], step_config: dict[str, Any]
    ) -> str:
        ...
        with self._lock:
            ...
            if self._trace_enabled:
                self._add_trace_event(
                    "checkpoint_created",
                    {"checkpoint_id": checkpoint_id, "workflow_id": workflow_id, "step_id": step_id},
                )

            return checkpoint_id

    def complete_checkpoint(
        self, checkpoint_id: str, state_after: dict[str, Any], execution_time: float, error: str | None = None
    ):
        ...
        with self._lock:
            ...
            if self._trace_enabled:
                self._add_trace_event(
                    ...
                    {"checkpoint_id": checkpoint_id, "execution_time": execution_time, "error": error},
                )
    ...
    def get_checkpoint_history(self, workflow_id: str | None = None) -> list[ExecutionCheckpoint]:
        """Get checkpoint history, optionally filtered by workflow."""
        with self._lock:
            checkpoints = [self._checkpoints[cp_id] for cp_id in self._checkpoint_history if cp_id in self._checkpoints]
    ...
    def create_breakpoint(
        ...
        with self._lock:
            ...
            if self._trace_enabled:
                self._add_trace_event(
                    "breakpoint_created",
                    {"breakpoint_id": id, "workflow_id": workflow_id, "step_id": step_id, "step_type": step_type},
                )
    ...
    def check_breakpoint(
        self, workflow_id: str, step_id: str, step_type: str, state: dict[str, Any]
    ...
    def inspect_state(self, workflow_id: str, path: str | None = None) -> dict[str, Any]:
    ...
    def get_execution_trace(self, limit: int | None = None) -> list[dict[str, Any]]:
    ...
    def add_checkpoint(
        self, workflow_id: str, step_id: str, state_before: dict[str, Any], step_config: dict[str, Any] = None
    ) -> str:
    ...
    def track_execution_flow(self, workflow_id: str, step_id: str, data: dict[str, Any]) -> None:
        ...
        if self._trace_enabled:
            self._add_trace_event("execution_flow", {"workflow_id": workflow_id, "step_id": step_id, "data": data})
