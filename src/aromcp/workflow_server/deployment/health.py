...
class HealthCheckManager:
    ...
    async def check_health(self) -> dict[str, Any]:
        ...
        # Check components
        health_status["components"]["executor"] = {"status": "healthy" if self.executor else "unavailable"}

        health_status["components"]["state_manager"] = {"status": "healthy" if self.state_manager else "unavailable"}

        health_status["components"]["observability"] = {"status": "healthy" if self.observability else "unavailable"}
    ...
    async def check_readiness(self) -> dict[str, Any]:
        ...
        readiness["checks"]["resource_availability"] = {"passed": True, "message": "Sufficient resources available"}

        # Configuration check
        readiness["checks"]["configuration"] = {"passed": True, "message": "Configuration valid"}

        # Overall readiness
        readiness["ready"] = all(check["passed"] for check in readiness["checks"].values())

        return readiness

    async def check_liveness(self) -> dict[str, Any]:
        """Check if system is alive and processing."""
        return {"alive": True, "last_activity": datetime.now(UTC).isoformat(), "processing_active": True}
