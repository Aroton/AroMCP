...
class ConfigLoader:
    ...
    def load_with_overrides(self, base_config: dict[str, Any]) -> dict[str, Any]:
        ...
        if "WORKFLOW_MAX_CONCURRENT" in os.environ:
            config.setdefault("workflow_server", {})["max_concurrent_workflows"] = int(
                os.environ["WORKFLOW_MAX_CONCURRENT"]
            )
        ...
        if "WORKFLOW_MONITORING_ENABLED" in os.environ:
            config.setdefault("workflow_server", {})["enable_monitoring"] = (
                os.environ["WORKFLOW_MONITORING_ENABLED"].lower() == "true"
            )
