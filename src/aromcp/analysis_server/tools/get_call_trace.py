...
def get_call_trace_impl(
    ...
    if not entry_point:
        errors.append(AnalysisError(code="INVALID_ENTRY_POINT", message="Entry point cannot be empty", file=None))
    ...
    if not entry_point or not existing_files:
        return CallTraceResponse(
            ...
            execution_paths=[],
            call_graph_stats=CallGraphStats(total_functions=0, total_edges=0, max_depth_reached=0, cycles_detected=0),
            errors=errors,
            total=0,
            page_size=None,
            next_cursor=None,
    ...
    try:
        ...
        graph_result = call_graph_builder.build_call_graph(entry_point, existing_files, max_depth)
        ...
        errors.append(
            AnalysisError(code="CALL_TRACE_ERROR", message=f"Error during call trace analysis: {str(e)}", file=None)
        )
