...
class FunctionAnalyzer:
    ...
    def analyze_function(self, function_name: str, file_path: str,
                        include_code: bool = True, include_types: bool = True,
                        ...
                        fallback_on_complexity: bool = False) -> tuple[FunctionDetail | None, list[AnalysisError]]:
        """
        ...
        try:
            ...
            types = {}
            if include_types:
                types, type_errors = self._extract_types(function_info, file_path, resolution_depth, 
                                                       resolve_imports, max_constraint_depth,
                                                       track_instantiations, resolve_conditional_types,
                                                       handle_recursive_types, fallback_on_complexity)
        ...
        try:
            ...
            if return_type_start < len(content) and content[return_type_start] == ':':
                ...
                while type_end < len(content):
                    ...
                    elif char == '=' and type_end + 1 < len(content) and content[type_end + 1] == '>' and brace_count == 0 and paren_count == 0 and angle_count == 0:
    ...
    def _find_class_method(self, content: str, class_name: str, method_name: str, 
    ...
    def _parse_single_parameter(self, param_str: str) -> ParameterType | None:
        ...
        try:
            ...
            return ParameterType(
                ...
                is_rest_parameter=is_rest
            )
            
        except Exception:
            return None
    
    def _extract_types(self, function_info: dict, file_path: str, 
                      resolution_depth: str, resolve_imports: bool = False,
                      max_constraint_depth: int = 3,
                      track_instantiations: bool = False,
                      resolve_conditional_types: bool = False,
                      handle_recursive_types: bool = False,
                      fallback_on_complexity: bool = False) -> tuple[dict[str, TypeDefinition], list[AnalysisError]]:
        """
            ...
            if type_annotation not in self.type_resolver.primitive_types:
                ...
                if base_type_name in self.type_resolver.builtin_generics and resolution_depth in ["generics", "full_inference", "full_type"]:
                    ...
                    type_def = self.type_resolver.resolve_type(
                        type_annotation, file_path, resolution_depth,
        ...
        if resolution_depth in ["generics", "full_inference", "full_type"]:
            ...
            if generic_params:
                constraint_types = self.type_resolver.extract_generic_constraints(
                    generic_params, file_path, resolution_depth,
                    max_constraint_depth=max_constraint_depth,
                    check_circular=True
                )
                    ...
                    if extends_match:
                        ...
                        if base_class not in types:
                            base_def = self.type_resolver.resolve_type(
                                base_class, file_path, resolution_depth,
                                max_constraint_depth=max_constraint_depth,
                                track_instantiations=track_instantiations,
                                resolve_conditional_types=resolve_conditional_types,
                                handle_recursive_types=handle_recursive_types
        ...
        if resolve_imports:
            ...
            for imported_type in imported_types:
                if imported_type not in types and imported_type not in self.type_resolver.primitive_types:
                    type_def = self.type_resolver.resolve_type(
                        imported_type, file_path, resolution_depth,
                        max_constraint_depth=max_constraint_depth,
                        track_instantiations=track_instantiations,
                        resolve_conditional_types=resolve_conditional_types,
                        handle_recursive_types=handle_recursive_types
            ...
            for local_type in local_types:
                ...
                if (local_type not in types and 
                    local_type not in self.type_resolver.primitive_types and
                    local_type not in imported_types):  # Avoid duplicating imported types
                    
                    type_def = self.type_resolver.resolve_type(
                        local_type, file_path, resolution_depth,
                        max_constraint_depth=max_constraint_depth,
                        track_instantiations=track_instantiations,
                        resolve_conditional_types=resolve_conditional_types,
                        handle_recursive_types=handle_recursive_types
    ...
    def _find_function_calls(self, function_info: dict, file_path: str) -> list[str]:
        ...
        try:
            ...
            for match in matches:
                ...
                if func_name not in ['if', 'for', 'while', 'switch', 'try', 'catch', 'return', 
                                   'typeof', 'function', 'async', 'await', 'new', 'throw',
                                   'const', 'let', 'var', 'constructor', 'super']:
    ...
    def analyze_multiple_functions(self, function_names: list[str], file_paths: list[str],
                                 **kwargs) -> dict[str, FunctionDetail]:
        """
