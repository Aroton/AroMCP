...
from ..models.typescript_models import CacheStats
from .memory_manager import CompressionStrategy, MemoryManager
...
class CacheInvalidationStrategy:
    ...
    def __init__(
        self, dependency_tracker: DependencyTracker, cache_manager: "CacheManager", enable_selective: bool = True
    ):
        self.dependency_tracker = dependency_tracker
        self.cache_manager = cache_manager
        self.enable_selective = enable_selective

    def invalidate_file(self, file_path: str, changed_exports: list[str] | None = None) -> list[str]:
...
class LRUCache(Generic[T]):
    ...
    def set(self, key: str, value: T, size_bytes: int = 0, dependencies: set[str] | None = None):
        ...
        with self._lock:
            ...
            while self.current_size_mb + entry_size_mb > self.max_size_mb and len(self.entries) > 0:
...
class FilesystemCache:
    ...
    def set(
        self, key: str, value: Any, size_bytes: int = 0, dependencies: set[str] | None = None, compress: bool = True
    ):
...
class CacheManager:
    ...
    def __init__(
        ...
        self.invalidation_strategy = CacheInvalidationStrategy(self.dependency_tracker, self, enable_selective=True)
    ...
    def set(self, key: str, value: Any, level: CacheLevel | None = None, dependencies: set[str] | None = None):
    ...
    def exists(self, key: str, level: CacheLevel | None = None) -> bool:
        ...
        if level:
            ...
            # Check all levels
            return any(cache.exists(key) for cache in self._level_caches.values() if cache)
    ...
    def get_detailed_stats(self) -> DetailedCacheStats:
        ...
        # Calculate totals
        self.detailed_stats.total_requests = self.detailed_stats.cache_hits + self.detailed_stats.cache_misses

        if self.detailed_stats.total_requests > 0:
            self.detailed_stats.hit_rate = self.detailed_stats.cache_hits / self.detailed_stats.total_requests
    ...
    def _collect_hot_keys(self) -> list[HotKey]:
        ...
        for level, cache in self._level_caches.items():
            if hasattr(cache, "entries"):
                ...
                sorted_entries = sorted(cache.entries.values(), key=lambda e: e.access_count, reverse=True)[:10]
    ...
    def check_health(self):
        ...
        if "memory_usage" in self._alert_thresholds:
            ...
            if "max" in thresholds and stats.cache_size_mb > thresholds["max"]:
                self.alert_handler(
                    f"Cache memory usage {stats.cache_size_mb:.1f}MB exceeds threshold {thresholds['max']:.1f}MB"
                )
        ...
        if "eviction_rate" in self._alert_thresholds:
            ...
            if stats.total_requests > 0:
                ...
                if "max" in thresholds and eviction_rate > thresholds["max"]:
                    self.alert_handler(
                        f"Cache eviction rate {eviction_rate:.1%} exceeds threshold {thresholds['max']:.1%}"
                    )
    ...
    def _promote_to_higher_levels(self, key: str, value: Any, found_level: CacheLevel):
        ...
        level_order = {CacheLevel.FILESYSTEM: 0, CacheLevel.SYMBOL: 1, CacheLevel.MEMORY: 2}
