...
class CallGraphBuilder:
    ...
    def build_call_graph(self, entry_point: str, file_paths: list[str], max_depth: int = 10) -> CallGraphResult:
        ...
        stats = CallGraphStats(total_functions=0, total_edges=0, max_depth_reached=0, cycles_detected=0)
        ...
        try:
            ...
            processing_time = (time.time() - start_time) * 1000
            return CallGraphResult(
                entry_point=entry_point, execution_paths=[], call_graph_stats=stats, processing_time_ms=processing_time
            )
            ...
            try:
                ...
                for pattern in patterns:
                    ...
                    for match in matches:
                        ...
                        self.function_definitions[func_name] = FunctionDefinition(
                            name=func_name, file=file_path, line=line_num, signature=match.group(0)
                        )
            ...
            try:
                ...
                for call_pattern in call_patterns:
                    ...
                    for match in matches:
                        ...
                        call_site = CallSite(
                            function_name=func_name, file=file_path, line=line_num, context=context.strip()
                        )
    ...
    def _build_graph_recursive(self, func_name: str, visited: set, current_depth: int, max_depth: int):
        ...
        for called_func in calls:
            ...
            if current_depth < max_depth:
                ...
                self._build_graph_recursive(called_func, new_visited, current_depth + 1, max_depth)
    ...
    def _find_calls_in_function(self, func_name: str) -> list[str]:
        ...
        try:
            with open(func_def.file, encoding="utf-8") as f:
                content = f.read()
    ...
    def _generate_execution_paths(self, entry_point: str, max_depth: int) -> list[ExecutionPath]:
        ...
        def dfs_paths(current_func: str, current_path: list[str], depth: int):
            ...
            if depth > max_depth:
                ...
                if len(current_path) > 1:
                    paths.append(ExecutionPath(path=current_path.copy(), condition=None, execution_probability=1.0))
                return
                ...
                if called_functions:  # If this function calls others
                    for called_func in called_functions:
                        ...
                        if called_func in current_path:
                            ...
                            paths.append(ExecutionPath(path=cycle_path, condition=None, execution_probability=1.0))
                        else:
                            # Continue exploring
        ...
        if not paths and entry_point in self.call_graph and self.call_graph[entry_point]:
            for called_func in self.call_graph[entry_point]:
                paths.append(ExecutionPath(path=[entry_point, called_func], condition=None, execution_probability=1.0))
