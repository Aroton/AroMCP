...
class TypeScriptParser:
    ...
    def __init__(self, cache_size_mb: int = 100, max_file_size_mb: int = 5, memory_manager: Any = None, enable_compression: bool = True, enable_string_interning: bool = True):
        """
    ...
    def parse_file(self, file_path: str, resolution_depth: str = ResolutionDepth.SYNTACTIC) -> ParseResult:
        ...
        if self._is_excluded_path(file_path):
            error = AnalysisError(
                code="EXCLUDED_PATH",
                message=f"File in excluded directory: {file_path}",
                file=file_path
            )
            return ParseResult(success=False, errors=[error])
    ...
    def _cache_result(self, file_path: str, tree: Any, content: str, parse_time_ms: float) -> None:
        ...
        while (self._cache_size_bytes + estimated_size > max_cache_bytes and 
               len(self._ast_cache) > 0):
    ...
    def _query_mock_nodes(self, tree: dict, node_type: str) -> list[Any]:
        ...
        node_types_map = {
            'function_declaration': [
                {'type': 'function_declaration', 'name': 'mockFunction', 'start_point': (1, 0), 'end_point': (5, 1), 'text': 'function mockFunction() {}'},
            ],
            'interface_declaration': [
                {'type': 'interface_declaration', 'name': 'MockInterface', 'start_point': (10, 0), 'end_point': (15, 1), 'text': 'interface MockInterface {}'},
            ],
            'class_declaration': [
                {'type': 'class_declaration', 'name': 'MockClass', 'start_point': (20, 0), 'end_point': (25, 1), 'text': 'class MockClass {}'},
            ],
            'type_alias_declaration': [
                {'type': 'type_alias_declaration', 'name': 'MockType', 'start_point': (30, 0), 'end_point': (31, 20), 'text': 'type MockType = string;'},
            ],
            'jsx_element': [
                {'type': 'jsx_element', 'start_point': (40, 0), 'end_point': (40, 20), 'text': '<div>Mock JSX</div>'},
            ],
            'jsx_self_closing_element': [
                {'type': 'jsx_self_closing_element', 'start_point': (45, 0), 'end_point': (45, 15), 'text': '<img src="test"/>'},
            ],
            'jsx_fragment': []
        }
    ...
    def cleanup_old_entries(self):
        ...
        self._cache_size_bytes = sum(
            len(str(entry.tree)) + 1024 
            for entry in self._ast_cache.values()
        )
