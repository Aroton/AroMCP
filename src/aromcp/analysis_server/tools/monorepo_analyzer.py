...
class WorkspaceContext:
    ...
    def find_references(self, symbol_name: str, project: str | None = None) -> list[ReferenceInfo]:
        ...
        for proj_name in projects_to_search:
            ...
            for file_path in project_obj.source_files:
                file_refs = self.symbol_resolver.find_symbol_references(symbol_name, file_path)
    ...
    def find_type_references(self, type_name: str) -> list[ReferenceInfo]:
        ...
        for project in self.projects.values():
            for file_path in project.source_files:
                type_refs = self.symbol_resolver.find_type_references(type_name, file_path)
    ...
    def analyze_project(self, project_name: str, isolated: bool = False) -> WorkspaceAnalysisResult:
        ...
        if project_name not in self.projects:
            return WorkspaceAnalysisResult(
                success=False,
                errors=[
                    AnalysisError(code="PROJECT_NOT_FOUND", message=f"Project '{project_name}' not found in workspace")
                ],
            )
            ...
            for file_path in project.source_files:
                try:
                    ...
                    for module_path in fallback_imports:
                        ...
                        if module_path.startswith("@") or any(
                            dep_name in module_path for dep_name in project.workspace_dependencies
                        ):
    ...
    def get_shared_type_stats(self) -> "SharedTypeStats":
        ...
        shared_types = {type_name: projects for type_name, projects in type_usage.items() if len(projects) > 1}
...
class MonorepoAnalyzer:
    ...
    def build_project_dependency_graph(self) -> ProjectDependencyGraph:
        ...
        if dep_graph.has_circular_dependencies():
            ...
            warnings.warn(f"Circular dependency detected between projects: {circular_deps}", UserWarning)
    ...
    def _find_tsconfig_files(self) -> list[str]:
        ...
        for root, dirs, files in os.walk(self.workspace_root):
            ...
            dirs[:] = [d for d in dirs if d not in {"node_modules", ".git", "dist", "build", "coverage", ".next"}]
    ...
    def _find_source_files(self, project_root: str, tsconfig: dict[str, Any]) -> list[str]:
        ...
        for root, dirs, files in os.walk(project_root):
            ...
            dirs[:] = [d for d in dirs if not any(self._matches_pattern(d, pattern) for pattern in exclude_patterns)]
