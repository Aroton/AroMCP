    ...
    for file_path in search_files:
        ...
        try:
            ...
            file_references = _find_symbol_references(
                symbol, file_path, lines, include_declarations, include_usages,
                include_confidence_scores, resolve_imports
            )
...
def _find_symbol_references(symbol: str, file_path: str, lines: list[str], 
                           include_declarations: bool, include_usages: bool,
                           include_confidence_scores: bool, resolve_imports: bool) -> list[ReferenceInfo]:
    ...
    if '#' in symbol:
        ...
        references.extend(_find_class_method_references(
            class_name, method_name, file_path, lines, 
            include_declarations, include_usages, include_confidence_scores
        ))
        return references
    ...
    for line_num, line in enumerate(lines, 1):
        ...
        if resolve_imports:
            ...
            if re.search(named_import_pattern, line) or re.search(default_import_pattern, line) or re.search(type_import_pattern, line):
                # Extract import path
        ...
        if include_usages:
            ...
            if (re.search(general_usage_pattern, line) and 
                not re.search(method_def_pattern, line)):  # Don't match method definitions
...
def _find_class_method_references(class_name: str, method_name: str, file_path: str, 
                                  lines: list[str], include_declarations: bool, 
                                  include_usages: bool, include_confidence_scores: bool) -> list[ReferenceInfo]:
...
def _resolve_inheritance_chains(symbol: str, file_paths: list[str], 
    ...
    for file_path in file_paths:
        ...
        try:
            ...
            for derived_class, base_class in matches:
                ...
                inheritance_chains.append(InheritanceChain(
                    base_class=base_class,
                    derived_classes=[derived_class],
                    file_path=file_path,
                    inheritance_depth=1
                ))
