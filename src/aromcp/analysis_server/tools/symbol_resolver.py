...
class SymbolResolver:
    ...
    def resolve_symbols(
        ...
        try:
            if resolution_pass == ResolutionPass.SYNTACTIC:
                result = self._syntactic_resolution(filtered_files, symbol_types, target_symbol, continue_on_error)
                ...
                error = AnalysisError(code="INVALID_INPUT", message=f"Unknown resolution pass: {resolution_pass}")
            ...
            result.analysis_stats.files_with_errors = len(
                [e for e in result.errors if e.code in ["PARSE_ERROR", "NOT_FOUND"]]
            )
            ...
            error = AnalysisError(code="RESOLUTION_ERROR", message=f"Symbol resolution failed: {e}")
            return SymbolResolutionResult(success=False, errors=[error])
    ...
    def _syntactic_resolution(
        self, file_paths: list[str], symbol_types: list[str] | None, target_symbol: str | None, continue_on_error: bool
    ) -> SymbolResolutionResult:
        ...
        for file_path in file_paths:
            try:
                ...
                file_symbols = self._extract_symbols_from_ast(parse_result.tree, file_path, symbol_types, target_symbol)
                ...
                error = AnalysisError(code="PARSE_ERROR", message=f"Failed to analyze {file_path}: {e}", file=file_path)
        ...
        if include_imports:
            try:
                ...
                import_result = self.import_tracker.build_dependency_graph(
                    file_paths=file_paths, include_external_modules=False
                )
                ...
                error = AnalysisError(code="IMPORT_RESOLUTION_ERROR", message=f"Failed to resolve imports: {e}")
                result.errors.append(error)
    ...
    def _dynamic_resolution(
        ...
        result = self._semantic_resolution(file_paths, symbol_types, target_symbol, True, continue_on_error)
        ...
        if resolve_inheritance:
            try:
                ...
                error = AnalysisError(
                    code="INHERITANCE_RESOLUTION_ERROR", message=f"Failed to resolve inheritance: {e}"
                )
                result.errors.append(error)
    ...
    def _extract_symbols_from_ast(
        self, tree: Any, file_path: str, symbol_types: list[str] | None, target_symbol: str | None
    ) -> dict[str, SymbolInfo]:
    ...
    def _extract_real_symbols(
        ...
        def traverse(node, class_context=None):
                ...
                for child in node.children:
                    if child.type == "variable_declarator":
                        ...
                        if identifier:
                            ...
                            elif (has_type_annotation or "test" in file_path.lower()) and (
                                not symbol_types or SymbolType.VARIABLE in symbol_types
                            ):
                                if not target_symbol or var_name == target_symbol:
                                    symbols[var_name] = SymbolInfo(
                                        name=var_name,
            ...
            # Extract methods within classes
                ...
                or node.type == "method_signature"
                or node.type == "abstract_method_signature"
            ) and class_context:
                            ...
                            for heritage_child in child.children:
                                if heritage_child.type == "extends_clause":
                                    for extends_child in heritage_child.children:
                                        if (
                                            extends_child.type == "identifier"
                                            or extends_child.type == "type_identifier"
                                        ):
    ...
    def _resolve_cross_file_references(
        self, symbols: dict[str, SymbolInfo], dependency_graph: Any, target_symbol: str | None
    ) -> list[ReferenceInfo]:
    ...
    def _resolve_inheritance_references(
        self, symbols: dict[str, SymbolInfo], inheritance_chains: list[InheritanceChain], target_symbol: str | None
    ) -> list[ReferenceInfo]:
    ...
    def _apply_pagination(self, result: SymbolResolutionResult, page: int, max_tokens: int) -> SymbolResolutionResult:
    ...
    def get_cache_stats(self) -> Any:
        ...
        return CacheStats(hits=self.cache_stats["hits"], misses=self.cache_stats["misses"], hit_rate=hit_rate)
    ...
    def get_file_symbols(self, file_path: str) -> list[SymbolInfo]:
        ...
        try:
            ...
            symbols_dict = self._extract_symbols_from_ast(parse_result.tree, file_path, None, None)
