...
class RuleMetadata(BaseModel):
    ...
    nextjs_api: list[str] = Field(
        ...
        description="Next.js API features: app-router, pages-router, api-routes, etc."
    )
...
class RuleCompression(BaseModel):
    ...
    pattern_extractable: bool = Field(
        default=True,
        description="Whether patterns can be extracted for minimal representation"
    )
    progressive_detail: list[str] = Field(
        default_factory=lambda: ["minimal", "standard", "detailed", "full"],
...
class RuleExamples(BaseModel):
    ...
    context_variants: dict[str, str] = Field(
        ...
        description="Context-specific variants (app_router, pages_router, etc.)"
    )
...
class EnhancedRule(BaseModel):
    ...
    import_map: list[dict[str, Any]] = Field(
        default_factory=list,
        description="Import mappings for the rule"
    )
    has_eslint_rule: bool = Field(
        default=False,
        description="Whether rule has ESLint equivalent"
    )
    relationships: dict[str, list[str]] = Field(
        default_factory=dict,
        description="Relationships: similar_rules, prerequisite_rules, etc."
    )
