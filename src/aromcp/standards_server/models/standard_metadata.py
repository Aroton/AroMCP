...
class ContextTriggers(BaseModel):
    ...
    architectural_layers: list[str] = Field(
        ...
        description="Architectural layers that need this standard"
    )
    ...
    import_indicators: list[str] = Field(
        default_factory=list,
        description="Import patterns that indicate need for this standard"
    )
    file_patterns: list[str] = Field(
        default_factory=list,
        description="File patterns that trigger this standard"
    )
    nextjs_features: list[str] = Field(
        default_factory=list,
        description="Next.js features that need this standard"
    )
...
class OptimizationHints(BaseModel):
    ...
    context_sensitive: bool = Field(
        default=True,
        description="Whether this standard depends on context"
    )
...
class EnhancedStandardMetadata(BaseModel):
    ...
    applies_to: list[str] = Field(
        default_factory=list,
        description="File types/patterns this applies to",
        alias="appliesTo"
    )
    ...
    dependencies: list[str] = Field(
        default_factory=list,
        description="Dependencies on other standards"
    )

    # Enhanced fields
    context_triggers: ContextTriggers = Field(
        default_factory=ContextTriggers,
        description="Context triggers for smart loading"
    )
    optimization: OptimizationHints = Field(
        default_factory=OptimizationHints,
        description="Optimization hints"
    )
    relationships: dict[str, list[str]] = Field(
        default_factory=dict,
        description="Relationships to other standards"
    )
    nextjs_config: dict[str, Any] = Field(
        default_factory=dict,
